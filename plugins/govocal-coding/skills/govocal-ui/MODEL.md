...
```html
<div id="page"></div>
<!-- Load BOTH, instances FIRST. The widgets engine composes canonical components by
     delegating to GV.render (e.g. extra-survey), so it depends on the instances layer. -->
<script src="../../skills/govocal-ui/govocal-instances.js"></script>
...
> Prototypes are self-contained: copy **`govocal-instances.js` + `govocal-widgets.js`** (and the
> CSS they need) into the prototype folder — the two are a pair (widgets composes canonical
> components via `GV.render`, so it needs the instances layer alongside it). Library pages
> reference the canonical files via `../../skills/govocal-ui/`.
