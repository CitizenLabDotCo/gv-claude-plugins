...
window.GV = (function () {
  ...
  // Identity normalisation — strip the RUNTIME-PRESENTATION slots before hashing, so the
  // provenance hash is a pure function of a component's STRUCTURE + authored content.
  // Avatar faces (random) and city logos (theme-reactive) are presentation, not identity —
  // the same category as the tenant colours, which never affect linked state because they
  // live in CSS vars, not in the markup. These two happen to be injected into innerHTML
  // (a background-image / an inline logo), so without this they'd leak into the hash and a
  // face-fill or a ?theme= logo swap would read as "modified". Normalising them out makes
  // them behave like the colours: deterministic linked-state, no re-stamping ever needed.
  var normalizeForHash = function (html) {
    if (typeof document === "undefined") return html;
    var tmp = document.createElement("div");
    tmp.innerHTML = html;
    tmp.querySelectorAll(".av, .gv-bo-table__avatar").forEach(function (el) {
      el.removeAttribute("style");      // random face background-image
      el.removeAttribute("data-face");  // fill-once guard flag
    });
    tmp.querySelectorAll("[data-gv-logo]").forEach(function (el) {
      el.innerHTML = "";                // theme-reactive city logo
    });
    // .gv-rail carousels: govocal-rail.js binds each rail on load and toggles edge-fade
    // classes on scroll/resize (data-railbound flag + rail--fade-left/right). That's pure
    // runtime presentation (overflow state), like avatar faces — strip it so a rail-bearing
    // section (the homepage list widgets) stays LINKED instead of reading "modified" the
    // moment the scroll handler runs.
    tmp.querySelectorAll(".gv-rail").forEach(function (el) {
      el.removeAttribute("data-railbound");
      el.classList.remove("rail--fade-left", "rail--fade-right");
      if (el.getAttribute("class") === "") el.removeAttribute("class");
    });
    // Live-editor slots: a [data-gv-slot] region is a runtime-content well (a builder's
    // palette / canvas / settings panel, continuously mutated by the editor engine). Like
    // avatar faces it is presentation, not identity — exclude its CONTENT from the hash so
    // the frame chrome around it stays LINKED while the editor freely rewrites the inside.
    tmp.querySelectorAll("[data-gv-slot]").forEach(function (el) {
      el.innerHTML = "";
    });
    return tmp.innerHTML;
  };

  // Tiny djb2 string hash (base-36) over the NORMALISED markup. Stamps data-gv-hash on a
  // rendered host so the Shift+C review overlay can tell a genuine, un-edited canonical
  // instance from one whose STRUCTURE was hand-edited after mount. Exposed as GV.hash so
  // the overlay hashes with the EXACT same function (stamp ⇄ check, no drift) — and so the
  // overlay inherits this normalisation for free (it calls window.GV.hash).
  var gvHash = function (s) {
    s = normalizeForHash(String(s == null ? "" : s));
  ...
  function hydrate(root) {
    ...
    // Fill avatar faces inside the hydration window so the provenance hash (taken right
    // after hydrate in mountAll) reflects the filled DOM. The fill is idempotent
    // (guarded by data-face), so the later page-level GVAvatars.fill(document.body) is a
    // no-op on these and won't re-mutate a stamped instance into "modified".
    if (window.GVAvatars && GVAvatars.fill) GVAvatars.fill(root);
  ...
  function mount(el, name, props) {
    ...
    hydrate(el);
    // Provenance: prove this subtree was produced by the canonical renderer (not just
    // markup wearing the same .gv-* classes). Stamp AFTER hydrate so the hash matches
    // the live, icon-filled DOM the overlay later re-hashes.
    el.setAttribute("data-gv-rendered", name);
    el.setAttribute("data-gv-hash", gvHash(el.innerHTML));
    return el;
  }

  var api = { render: render, mount: mount, hydrate: hydrate, register: register, components: registry, esc: esc, hash: gvHash };
...
/* ── bo-tabs ──────────────────────────────────────────────────────────────── */
/* bo-tabs.js — canonical instance renderer for the back-office tab strip (.gv-bo-tabs).
 * GV.render("bo-tabs", props) → canonical HTML string.
 *
 * The SAME tab component appears in several back-office contexts (govocal-bo.css),
 * selected by `variant` (each appends .gv-bo-tabs--<variant>):
 *   • project editor header   — variant "project" (.gv-bo-tabs--project): the strip
 *                               that sits flush under bo-topbar in the Project Editor.
 *                               It is a standalone SIBLING of bo-topbar, NOT nested.
 *   • top-level section strip — variant "top"  (.gv-bo-tabs--top: Dashboard/Settings…)
 *   • phase sub-tabs          — variant "sub"  (.gv-bo-tabs--sub)
 *   • bare elevated strip      — no variant (the base .gv-bo-tabs card)
 * A fully self-contained component: it composes nothing and assumes no parent.
 * The Project Editor chrome is the bo-app-shell PATTERN, which composes this strip
 * and bo-topbar as siblings.
 *
 * Props (all optional):
 *   tabs     Array<{label, active?, new?, href?, data?}>   default: the six project tabs
 *            data is a map of per-tab data-* attrs, e.g. {sub:"setup", tabkey:"setup"}
 *            → data-sub="setup" data-tabkey="setup" (the sub-tab panels switch on these).
 *   variant  "project" | "top" | "sub" | null   appends .gv-bo-tabs--<variant>
 *   label    string   the nav aria-label   default: "Project"
 *   id       string   id on the <nav> (sub-tab handlers target some by id)
 *   bleed    boolean  full-width card-bleed: break the strip out of the card's
 *            --gv-space-10 padding so the underline spans edge to edge (community-monitor
 *            settings sub-tabs). `style` overrides it with a raw inline style if given.
 *   style    string   raw inline style on the <nav> (escape hatch / overrides bleed)
 */
...
var BO_TABS_BLEED_STYLE = "margin:0 calc(-1 * var(--gv-space-10)) var(--gv-space-6); padding:0 var(--gv-space-10)";

GV.register("bo-tabs", function (props) {
  props = props || {};
  var esc = GV.esc;

  var tabs    = props.tabs || BO_TABS_DEFAULT;
  var variant = props.variant ? " gv-bo-tabs--" + esc(props.variant) : "";
  var label   = props.label != null ? props.label : "Project";
  var idAttr  = props.id != null ? ' id="' + esc(props.id) + '"' : "";
  var styleAttr = props.style != null ? ' style="' + esc(props.style) + '"'
                : (props.bleed ? ' style="' + BO_TABS_BLEED_STYLE + '"' : "");

  var tabsHtml = tabs.map(function (t) {
    var cls = "gv-bo-tab" + (t.active ? " is-active" : "");
    var badge = t.new ? '<span class="gv-bo-tab__new">NEW</span>' : "";
    var data = "";
    if (t.data) for (var k in t.data) {
      if (Object.prototype.hasOwnProperty.call(t.data, k) && t.data[k] != null)
        data += " data-" + esc(k) + '="' + esc(t.data[k]) + '"';
    }
    return '<a class="' + cls + '" href="' + esc(t.href || "#") + '"' + data + '>' + esc(t.label) + badge + '</a>';
  }).join("\n          ");

  return (
    '<nav class="gv-bo-tabs' + variant + '"' + idAttr + ' aria-label="' + esc(label) + '"' + styleAttr + '>\n' +
    '          ' + tabsHtml + '\n' +
    '        </nav>'
  );
});
...
/* ── bo-topbar ────────────────────────────────────────────────────────────── */
/* bo-topbar.js — canonical instance renderer for the back-office project top bar.
 * GV.render("bo-topbar", props) → canonical HTML string.
 *
 * The elevated #FBFBFB header panel that opens the Project Editor: the project
 * title, the status/meta row, and the action buttons — and ONLY that. It is a
 * fully self-contained component that composes nothing. The project tab row is a
 * SEPARATE component (bo-tabs); the Project Editor chrome is the bo-app-shell
 * PATTERN, which places bo-topbar and bo-tabs side by side as siblings.
 *
 * BO surface — uses the fixed neutral BO palette via .gv-bo (NEVER tenant-themed).
 *
 * Props (all optional — defaults reproduce the primary HEAD instance):
 *   title    string   default: "Let's Reimagine Dorothea Dix Park"
 *   meta     Array<{icon, label}>   default: the four HEAD items
 *   actions  raw HTML string   the .gv-bo-topbar__actions content (default HEAD)
 */
...
var BO_TOPBAR_ACTIONS =
  '<button class="gv-btn secondary-outlined icon-only" aria-label="Preview project"><span data-gv-icon="eye"></span></button>' +
...
GV.register("bo-topbar", function (props) {
  props = props || {};
  var esc = GV.esc;

  var title   = props.title   != null ? props.title   : "Let's Reimagine Dorothea Dix Park";
  var meta    = props.meta    || BO_TOPBAR_META;
  var actions = props.actions != null ? props.actions : BO_TOPBAR_ACTIONS;
...
/* ── bo-builder-frame ─────────────────────────────────────────────────── */
/* bo-builder-frame.js — canonical instance renderer for the back-office EDITOR FRAME:
 * the `.gv-bo-cb` 3-pane builder shell shared by all four builders
 * (content / homepage / report / survey). They are the same frame — a topbar over a
 * left WIDGET PALETTE, a center CANVAS, and a right SETTINGS panel — and differ only in
 * what fills those three panes (the widget catalog, the canvas model, the per-widget
 * settings). So the FRAME is one master; the pane contents are per-builder slots.
 *
 * Live-editor boundary: the canvas (and the palette it is built into, and the settings
 * body swapped on selection) are continuously mutated by each builder's editor engine
 * (GVPageBuilder for content/homepage; bespoke DOM/Model code for report/survey). A hash
 * over that moving content could never stay LINKED — so each pane (and the topbar actions,
 * which carry a mutable status pill on the survey builder) is rendered inside a
 * `data-gv-slot` well whose CONTENT is excluded from the provenance hash (see
 * normalizeForHash). The LOCKED identity is therefore the shared skeleton: the `.gv-bo-cb`
 * root, the topbar back-button + titles, the `.gv-bo-cb__body`, and the three pane
 * wrappers (`.gv-bo-cb-palette` / `.gv-bo-cb-canvas` / `.gv-bo-cb-settings`). Each builder
 * fills the slots with its own markup + engine and the host stays LINKED while editing.
 *
 * HOST = the `.gv-bo-cb` element itself. This master renders only its INNER (the topbar +
 * the `.gv-bo-cb__body` three-pane row) — so the builder engines that toggle STATE classes
 * on `.gv-bo-cb` (`.cb-has-panel`, `.cb-preview`, `.rb-has-panel`…) change the host's OWN
 * class, which is outside `host.innerHTML` and so never affects the provenance hash. Mount
 * with `GV.mount('#cb-root', 'bo-builder-frame', {...})` where `#cb-root` is the page's
 * `<div class="gv-bo-cb" id="cb-root">`.
 *
 * Props (all optional):
 *   title     string   topbar H1
 *   subtitle  string   topbar sub-line (omitted if null)
 *   back      {id, testid, label}   back button (label → aria-label, default "Go back")
 *   actions   rawHTML  the `.gv-bo-cb-topbar__actions` content (locale/preview/view/save,
 *                      survey status pill…) — per-builder; in a slot so a mutable pill is ok
 *   palette   {label, html}   the `.gv-bo-cb-palette` aside-label + inner (slot)
 *   canvas    {class, html}   extra class on `.gv-bo-cb-canvas` (e.g. "rb-canvas") + inner (slot)
 *   settings  {id, label, html}  the `.gv-bo-cb-settings` id + aria-label + full inner (slot:
 *                      close button + header + body — passed verbatim so no wrapper is added)
 */
GV.register("bo-builder-frame", function (props) {
  props = props || {};
  var esc = GV.esc;

  var back   = props.back || {};
  var backAttrs =
    (back.id ? ' id="' + esc(back.id) + '"' : "") +
    (back.testid ? ' data-testid="' + esc(back.testid) + '"' : "");
  var backLabel = back.label != null ? back.label : "Go back";

  var title    = props.title != null ? props.title : "";
  var subtitle = props.subtitle;
  var actions  = (props.actions != null ? props.actions : "");

  var palette  = props.palette || {};
  var paletteLabel = palette.label != null ? ' aria-label="' + esc(palette.label) + '"' : "";

  var canvas   = props.canvas || {};
  var canvasClass = canvas.class ? " " + esc(canvas.class) : "";

  var settings = props.settings || {};
  var settingsId    = settings.id != null ? ' id="' + esc(settings.id) + '"' : "";
  var settingsLabel = ' aria-label="' + esc(settings.label != null ? settings.label : "Block settings") + '"';

  return (
    '<header class="gv-bo-cb-topbar">' +
      '<button class="gv-bo-cb-topbar__back" type="button"' + backAttrs + ' aria-label="' + esc(backLabel) + '">' +
        '<span class="gv-icon" data-gv-icon="arrow-left"></span>' +
      '</button>' +
      '<div class="gv-bo-cb-topbar__titles">' +
        '<h1 class="gv-bo-cb-topbar__title">' + esc(title) + '</h1>' +
        (subtitle != null ? '<p class="gv-bo-cb-topbar__sub">' + esc(subtitle) + '</p>' : "") +
      '</div>' +
      '<div class="gv-bo-cb-topbar__actions" data-gv-slot>' + actions + '</div>' +
    '</header>' +
    '<div class="gv-bo-cb__body">' +
      '<aside class="gv-bo-cb-palette"' + paletteLabel + ' data-gv-slot>' + (palette.html || "") + '</aside>' +
      '<main class="gv-bo-cb-canvas' + canvasClass + '" data-gv-slot>' + (canvas.html || "") + '</main>' +
      '<aside class="gv-bo-cb-settings"' + settingsId + settingsLabel + ' data-gv-slot>' + (settings.html || "") + '</aside>' +
    '</div>'
  );
});
...
/* ── bo-table ─────────────────────────────────────────────────────────── */
/* bo-table.js — canonical instance renderer for the back-office data table.
 *
 * The `.gv-bo-table` family: the dense, neutral admin tables behind every BO
 * surface (dashboard feed/management, projects list, users/seats/groups, the
 * input-manager posts grid, and the method participants/results tables). It is
 * the BO counterpart to the FO `.gv-table` primitive — different markup, the
 * fixed BO palette via `.gv-bo` (never tenant-themed). A self-contained
 * component: it renders the table chrome (element, variant classes, optional
 * `<colgroup>`, `<thead>`) and wraps the body rows; the ROW DATA is the
 * consumer's (passed as `rows`/`tbody`), since every real table is data-driven.
 *
 * Every real BO table ships an EMPTY `<tbody>` in static HTML and fills it from
 * page data at runtime — so to LOCK one, the page hands its already-built body
 * HTML to this master (via `tbody`) or its rows (via `rows`) and mounts once;
 * the master then owns the chrome and the body lives inside the stamped host, so
 * the provenance hash is stable (checkbox/`<select>` state changes don't touch
 * innerHTML, so they stay LINKED). Sortable tables that toggle a header class on
 * click must RE-MOUNT with the new `sorted` column (see bo-method-* pages).
 *
 * Props (all optional):
 *   variant   string | string[]   → appended as `is-<v>` classes. The real
 *                                    variants: "list" (is-list, zebra rows) and
 *                                    "bordered" (is-bordered, outlined card).
 *   tableClass string             → extra raw classes on <table> (e.g. "us-table")
 *   aria      string              → aria-label on <table>
 *   colgroup  Array<string|{class,style}>  → emits <colgroup><col …>
 *   head      string | Array      → <thead> content. A raw string is used verbatim
 *                                    as the thead inner (one or more <tr>); an array
 *                                    is one header row of CELLS (see headCell below).
 *   bodyId    string              → id on <tbody> (keep the page's original id so
 *                                    other handlers that query it still resolve)
 *   tbody     string              → raw <tbody> inner HTML (the escape hatch: pass
 *                                    the page's already-computed body — byte-faithful)
 *   rows      Array               → body rows when not using `tbody`. Each item:
 *                                    • string  → a full "<tr>…</tr>" (used verbatim;
 *                                      if it doesn't start with <tr it is wrapped)
 *                                    • {tr}     → verbatim <tr> string
 *                                    • {cells, class, attrs} → built from CELLS
 *
 * Header cell (an item of the `head` array):
 *   • string                      → <th> with escaped text
 *   • { raw }                     → used verbatim (full <th> escape hatch)
 *   • { label?, html?, class?, style?, check?, num?, sort?, sorted?, chevron? }
 *       check  → adds .gv-bo-table__check ;  num → adds .gv-bo-table__num
 *       sort   → adds .gv-bo-th--sort     ;  sorted → adds .is-sorted (+ chevron)
 *       html   → raw inner (e.g. a header checkbox / icon group) instead of label
 *
 * Body cell (an item of a row's `cells` array):
 *   • string                      → raw HTML inner of a <td> (HTML-capable)
 *   • { raw }                     → used verbatim (full <td>/<th> escape hatch)
 *   • { html?, text?, class?, style?, check?, num?, opts?, colspan? }
 *       check → .gv-bo-table__check ; num → .gv-bo-table__num ; opts → .gv-bo-table__opts
 *       text  → escaped text inner ;  html → raw inner
 */
GV.register("bo-table", function (props) {
  props = props || {};
  var esc = GV.esc;

  /* variant(s) → is-* classes */
  var vs = props.variant;
  if (vs == null) vs = [];
  else if (typeof vs === "string") vs = vs.split(/\s+/);
  var cls = "gv-bo-table";
  vs.forEach(function (v) { if (v) cls += " is-" + esc(v); });
  if (props.tableClass) cls += " " + esc(props.tableClass);
  var aria = props.aria != null ? ' aria-label="' + esc(props.aria) + '"' : "";

  /* colgroup */
  var colHtml = "";
  if (props.colgroup && props.colgroup.length) {
    colHtml = "<colgroup>" + props.colgroup.map(function (c) {
      if (c == null) return "<col>";
      if (typeof c === "string") return '<col class="' + esc(c) + '">';
      var a = "";
      if (c.class) a += ' class="' + esc(c.class) + '"';
      if (c.style) a += ' style="' + esc(c.style) + '"';
      return "<col" + a + ">";
    }).join("") + "</colgroup>";
  }
  ...
  /* header cell */
  function headCell(h) {
    if (h == null) return "<th></th>";
    if (typeof h === "string") return "<th>" + esc(h) + "</th>";
    if (h.raw != null) return h.raw;
    var c = h.class ? String(h.class) : "";
    if (h.check)  c = ("gv-bo-table__check " + c).replace(/\s+$/, "");
    if (h.num)    c = (c + " gv-bo-table__num").replace(/^\s+/, "");
    if (h.sort)   c = (c + " gv-bo-th--sort").replace(/^\s+/, "");
    if (h.sorted) c = (c + " is-sorted").replace(/^\s+/, "");
    var a = "";
    if (c)        a += ' class="' + esc(c) + '"';
    if (h.style)  a += ' style="' + esc(h.style) + '"';
    var inner;
    if (h.html != null) inner = h.html;
    else {
      inner = h.label != null ? esc(h.label) : "";
      if (h.sorted || h.chevron) inner += ' <span class="gv-icon" data-gv-icon="chevron-down"></span>';
    }
    return "<th" + a + ">" + inner + "</th>";
  }

  var headHtml = "";
  if (props.head != null) {
    if (typeof props.head === "string") headHtml = "<thead>" + props.head + "</thead>";
    else headHtml = "<thead><tr>" + props.head.map(headCell).join("") + "</tr></thead>";
  }

  /* body cell (for rows given as {cells:[…]}) */
  function bodyCell(cl) {
    if (cl == null) return "<td></td>";
    if (typeof cl === "string") return "<td>" + cl + "</td>";
    if (cl.raw != null) return cl.raw;
    var c = cl.class ? String(cl.class) : "";
    if (cl.check) c = ("gv-bo-table__check " + c).replace(/\s+$/, "");
    if (cl.num)   c = (c + " gv-bo-table__num").replace(/^\s+/, "");
    if (cl.opts)  c = (c + " gv-bo-table__opts").replace(/^\s+/, "");
    var a = "";
    if (c)          a += ' class="' + esc(c) + '"';
    if (cl.style)   a += ' style="' + esc(cl.style) + '"';
    if (cl.colspan) a += ' colspan="' + esc(cl.colspan) + '"';
    var inner = cl.html != null ? cl.html : (cl.text != null ? esc(cl.text) : "");
    return "<td" + a + ">" + inner + "</td>";
  }

  /* body */
  var bodyInner;
  if (props.tbody != null) {
    bodyInner = props.tbody;
  } else if (props.rows && props.rows.length) {
    bodyInner = props.rows.map(function (r) {
      if (r == null) return "";
      if (typeof r === "string") return /^\s*<tr[\s>]/.test(r) ? r : "<tr>" + r + "</tr>";
      if (r.tr != null) return r.tr;
      var ra = "";
      if (r.class) ra += ' class="' + esc(r.class) + '"';
      if (r.attrs) ra += " " + r.attrs;
      var cells = (r.cells || []).map(bodyCell).join("");
      return "<tr" + ra + ">" + cells + "</tr>";
    }).join("");
  } else {
    bodyInner = "";
  }
  var bodyId = props.bodyId != null ? ' id="' + esc(props.bodyId) + '"' : "";

  return (
    '<table class="' + cls + '"' + aria + ">" +
      colHtml + headHtml +
      "<tbody" + bodyId + ">" + bodyInner + "</tbody>" +
    "</table>"
  );
});
 ...
 *   <label class="gv-checkbox"><input type="checkbox" checked /><span class="box"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 16.17 4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg></span> Checked</label>
 *
...
GV.register("checkbox-radio", function (props) {
  ...
  return '<label class="gv-checkbox"><input type="checkbox"' + checkedAttr + disabledAttr + ' /><span class="box"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 16.17 4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg></span> ' + esc(label) + "</label>";
});
...
GV.register("event-card", function (props) {
  ...
  var cardsHtml = cards.map(function (card) {
    ...
    // Optional media PHOTO (card.img): replaces the decorative calendar glyph with a
    // real, alt-described image; the date chip + RSVP overlay it. No img → calendar icon
    // (unchanged default). Backward-compatible.
    var bordered = (card.bordered || props.bordered) ? " bordered" : "";
    var mediaInner = card.img
      ? '<img src="' + esc(card.img) + '" alt="' + esc(card.imgAlt || "") + '" loading="lazy" />'
      : '<span data-gv-icon="calendar"></span>';
    var mediaOpen = card.img
      ? '<div class="gv-event-card__media">'              // a captioned photo carries its own alt
      : '<div class="gv-event-card__media" aria-hidden="true">';

    return (
      '<article class="gv-event-card' + bordered + '">' +
        mediaOpen +
          mediaInner +
...
GV.register("header-nav", function (props) {
  ...
  /* ─────────────────────────────────────────────────────────────
     SIGNED-OUT (primary)
     ───────────────────────────────────────────────────────────── */
  { /* signed-out render — shared; the "switcher" variant wraps this + the signed-in one */
    var authSetOut = state === "switcher" ? " data-auth-set=\"in\"" : "";
        ...
        "<div class=\"gv-header__inner\">" +
          ...
          "<div class=\"gv-header__actions\">" +
            "<button class=\"gv-iconbtn gv-desktop-only\" aria-label=\"Suche\">" +
              "<span data-gv-icon=\"search\"></span>" +
            "</button>" +
            "<button class=\"gv-btn primary gv-desktop-only\" style=\"padding:8px 18px;line-height:20px\"" + authSetOut + ">" +
              esc(ctaLabel) +
            "</button>" +
            "<details class=\"gv-nav-m\">" +
              ...
              "<nav class=\"gv-nav-m__panel\" aria-label=\"Hauptnavigation (mobil)\">" +
                ...
                "<button class=\"gv-btn primary full\"" + authSetOut + ">" + esc(ctaLabel) + "</button>" +
              "</nav>" +
            "</details>" +
          "</div>" +
        "</div>" +
  ...
  var authSetIn = state === "switcher" ? " data-auth-set=\"out\"" : "";
  ...
  var acctMenuHtml = accountMenu.map(function (it) {
    if (it.type === "button") {
      return "<button type=\"button\"" + authSetIn + ">" + esc(it.label) +
        ...
        "<div class=\"gv-header__actions\">" +
          ...
          "<details class=\"gv-nav-m\">" +
            ...
            "<nav class=\"gv-nav-m__panel\" aria-label=\"Hauptnavigation (mobil)\">" +
              ...
              mobileLinkInHtml +
              "<a class=\"gv-nav-m__search\" href=\"#\"><span data-gv-icon=\"search\" aria-hidden=\"true\"></span> Suche</a>" +
              "<button class=\"gv-btn primary-outlined full\" style=\"margin-top:14px\"" + authSetIn + ">Abmelden</button>" +
            "</nav>" +
          "</details>" +
        "</div>" +
      "</div>" +
  ...
  /* switcher — render BOTH states; the matching one shows per the [data-auth] ancestor
     (canonical .gv-auth-* rules). The signed-out CTA carries data-auth-set="in" and the
     signed-in account "sign out" carries data-auth-set="out", so the page's auth-toggle JS
     flips between them. display:contents keeps each <header> a direct flow child (sticky). */
  if (state === "switcher") {
    return '<div class="gv-auth-out" style="display:contents">' + outHTML + '</div>' +
           '<div class="gv-auth-in" style="display:contents">' + inHTML + '</div>';
  }
  return state === "signed-in" ? inHTML : outHTML;
});
...
GV.register("hero", function (props) {
  ...
  if (variant === "photo") {
    ...
    /* Set the photo BOTH as the --gv-hero-image custom property (for any consumer
       that reads it) AND as a direct inline background-image. The latter is what
       actually paints: a relative url() inside a custom property is resolved by the
       browser against the stylesheet that consumes it (govocal-ui.css), not the page,
       so a page-relative imageUrl would 404; an inline background-image resolves
       against the document and works from any page. */
    return (
      '<section class="gv-hero" style="--gv-hero-image:url(\'' + esc(imageUrl) + '\');background-image:url(\'' + esc(imageUrl) + '\')">' +
        '<div class="gv-hero__inner">' +
          '<h1 class="gv-hero__title">' + esc(title) + '</h1>' +
          '<p class="gv-hero__lead">' + esc(lead) + '</p>' +
        '</div>' +
  ...
  var outBtnHref    = props.outBtnHref   != null ? props.outBtnHref   : "";
  var credit        = props.credit       != null ? props.credit       : "";
  ...
  /* Paint the photo via a direct inline background-image (not only the
     --gv-hero-image custom property): a relative url() in a custom property resolves
     against the consuming stylesheet (govocal-ui.css), not the page, so a page-relative
     imageUrl 404s; an inline background-image resolves against the document. The tworow
     layout deliberately uses background-image:none + the .gv-hero__media <img>, so only
     the full-bleed layouts (full/fixed) get the inline background-image. */
  var heroStyle = "--gv-hero-image:url('" + esc(imageUrl) + "')" +
    (layout === "tworow" ? "" : ";background-image:url('" + esc(imageUrl) + "')");
    ...
    '<section class="gv-hero signed-out centered layout-' + esc(layout) + '" id="switchHero" data-testid="' + esc(testid) + '"' +
      ...
      '<div class="gv-hero__inner">' +
        ...
        '<div class="banner-content" data-when="out">' +
          ...
          '</div>' +
          '<div class="gv-hero__actions">' + (outBtnHref
            ? '<a class="gv-btn primary-inverse" href="' + esc(outBtnHref) + '">' + esc(outBtnLabel) + '</a>'
            : '<button type="button" class="gv-btn primary-inverse">' + esc(outBtnLabel) + '</button>') + '</div>' +
        '</div>' +
        ...
        '</div>' +
      '</div>' +
      (credit ? '<span class="gv-hero__credit">' + esc(credit) + '</span>' : '') +
    '</section>'
  );
});
...
GV.register("poll", function (props) {
  ...
  var fieldsets = questions.map(function (q, i) {
    ...
    var opts = (q.options || []).map(function (opt) {
      if (isCheckbox) {
        return (
          '<label class="gv-checkbox e2e-poll-option"><input type="checkbox" name="' + esc(q.name) + '"><span class="box"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 16.17 4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg></span> ' + esc(opt) + '</label>'
        );
      }
      return (
...
GV.register("project-card", function (props) {
  ...
  if (variant === "boxed-grid") {
    ...
    // feat (default true): the homepage feature+grid layout (.gv-pgrid--feat, span-3
    // feature, span-half standards). feat:false → a uniform 3-up index grid (.gv-pgrid,
    // no spans) for the projects-list page. Backward-compatible: default keeps --feat.
    var feat = props.feat !== false;
    ...
    function renderFoot(foot) {
      ...
      if (foot.type === "poststat") {
        // Generic stat line: any unit ("votes", "contributions", …); closed:true dims it.
        return '<div class="gv-pcard__foot">' +
          '<span class="gv-poststat' + (foot.closed ? " closed" : "") + '"><b>' + esc(foot.count) + '</b>&nbsp;' + esc(foot.unit || "contributions") + '</span>' +
          "</div>";
      }
      return "";
    }
    ...
    var articlesHtml = boxedCards.map(function (card) {
      ...
      /* article class — in a uniform (feat:false) grid, standard cards carry no span. */
      var spanCls = "";
      if (!isFeature) {
        var sp = card.span !== undefined ? card.span : (feat ? "half" : "");
      ...
      return '<article class="' + articleCls + '">' +
        ...
        thumbHtml +
        '<div class="gv-pcard__body">' +
          // Optional category·area meta line above the title (e.g. "Survey · Centre").
          (card.meta ? '<span class="gv-pcard__meta">' + esc(card.meta) + "</span>" : "") +
          '<h3 class="gv-pcard__title"><a href="' + esc(card.href || "#") + '">' + esc(card.title) + "</a></h3>" +
          (card.desc ? '<p class="gv-pcard__desc">' + esc(card.desc) + "</p>" : "") +
    ...
    return '<div class="gv-pgrid' + (feat ? " gv-pgrid--feat" : "") + '">' + articlesHtml + "</div>";
  }
...
/* ── open-to-participation / finished-or-archived / areas ───────────────────────
 * The three homepage-builder "list" widgets as LOCKABLE canonical instances, named
 * to match the real product's Homepage builder widgets (Pages & menu → Home → Edit):
 *   open-to-participation  → "Open to participation"  (clock status + Learn more)
 *   finished-or-archived   → "Finished projects"      (green-check status + Learn more)
 *   areas                  → "In your area"           (participant bubbles + a short desc)
 * These mirror the GVWidgets.homepage list widgets (compactCard / teamCard / railSec in
 * govocal-widgets.js) so the reference homepage can LOCK each section to ONE master
 * instead of hand-authoring .gv-pcard markup (which reads BESPOKE). Same coexistence
 * pattern as `spotlight` (GV master + GVWidgets block share the registry name). Each
 * takes its content as a `cards` array prop; defaults reproduce the homepage rows.
 * data-gv-icon glyphs are filled inside GV.hydrate BEFORE mountAll stamps the hash, so
 * they stay LINKED. */

/* compact light card with a status line (clock by default; green check when done) +
   the section shell (title + horizontal rail). Inlined per-register (no free-floating
   helpers) so the registry's prop extractor attributes props to the right component. */
GV.register("open-to-participation", function (props) {
  props = props || {};
  var esc = GV.esc;
  function compact(c, done) {
    var href = c.href != null ? c.href : "#";
    var icon = c.statusIcon != null ? c.statusIcon : (done ? "check-circle" : "clock-solid");
    var more = c.more != null ? c.more : "Learn more";
    return '<article class="gv-pcard light compact">' +
        '<div class="gv-pcard__thumb"><img src="' + esc(c.thumb || "") + '" alt="' + esc(c.thumbAlt || "") + '" loading="lazy" /></div>' +
        '<div class="gv-pcard__body">' +
          '<h3 class="gv-pcard__title"><a href="' + esc(href) + '">' + esc(c.title || "") + '</a></h3>' +
          '<span class="gv-pcard__status' + (done ? ' done' : '') + '"><span data-gv-icon="' + esc(icon) + '" aria-hidden="true"></span> ' + esc(c.status || "") + '</span>' +
          '<span class="gv-pcard__more">' + esc(more) + '</span>' +
        '</div></article>';
  }
  function rail(title, hid, cardsHtml) {
    var aria = props.ariaLabel != null ? props.ariaLabel : (title + ", scroll for more");
    return '<section class="gv-section" aria-labelledby="' + esc(hid) + '">' +
        '<div class="gv-section__head"><h2 class="gv-title h2" id="' + esc(hid) + '">' + esc(title) + '</h2></div>' +
        '<div class="gv-section__body"><div class="gv-rail" tabindex="0" role="group" aria-label="' + esc(aria) + '">' + cardsHtml + '</div></div>' +
      '</section>';
  }
  var DEFAULT = [
    { title: "Riverside path study",       status: "2 days left",   thumb: "img/street.webp",           thumbAlt: "A quiet tree-lined road along the Riverside path" },
    { title: "Harbour cycle network",      status: "5 days left",   thumb: "img/climate-harbour.webp",  thumbAlt: "The harbour waterfront with moored boats" },
    { title: "Redesigning Market Square",  status: "1 week left",   thumb: "img/done-square.webp",      thumbAlt: "Market Square with people and cyclists" },
    { title: "Northgate green corridor",   status: "2 months left", thumb: "img/climate-northgate.webp",thumbAlt: "A leafy residential street in Northgate" },
    { title: "Downtown mobility plan",     status: "No end date",   thumb: "img/feat-downtown.webp",    thumbAlt: "A busy downtown street with shops and pedestrians" }
  ];
  var cards = props.cards !== undefined ? props.cards : DEFAULT;
  var title = props.title != null ? props.title : "Open to participation";
  return rail(title, props.headingId != null ? props.headingId : "open-title", cards.map(function (c) { return compact(c, 0); }).join(""));
});

GV.register("finished-or-archived", function (props) {
  props = props || {};
  var esc = GV.esc;
  function compact(c, done) {
    var href = c.href != null ? c.href : "#";
  ...
  var DEFAULT = [
    { title: "Children & youth strategy 2025–2030", status: "7 weeks ago",  thumb: "img/done-youth.webp",       thumbAlt: "Young residents taking part in a community workshop" },
    { title: "Greening the schoolyards",            status: "3 weeks ago",  thumb: "img/done-schoolyards.webp", thumbAlt: "A green, tree-filled park where a schoolyard was regreened" },
    { title: "Old Town as new",                     status: "5 months ago", thumb: "img/done-oldtown.webp",     thumbAlt: "A restored cobbled street in the Old Town" }
  ];
  var cards = props.cards !== undefined ? props.cards : DEFAULT;
  var title = props.title != null ? props.title : "Completed projects";
  var hid = props.headingId != null ? props.headingId : "done-title";
  var aria = props.ariaLabel != null ? props.ariaLabel : (title + ", scroll for more");
  return '<section class="gv-section" aria-labelledby="' + esc(hid) + '">' +
      '<div class="gv-section__head"><h2 class="gv-title h2" id="' + esc(hid) + '">' + esc(title) + '</h2></div>' +
      '<div class="gv-section__body"><div class="gv-rail" tabindex="0" role="group" aria-label="' + esc(aria) + '">' +
        cards.map(function (c) { return compact(c, 1); }).join("") +
      '</div></div></section>';
});

GV.register("areas", function (props) {
  props = props || {};
  var esc = GV.esc;
  var DEFAULT = [
    { title: "Riverside climate team", count: "145", thumb: "img/climate-riverside.webp", thumbAlt: "A tree-lined cycle avenue in the Riverside district",  desc: "Wanted: your idea for a better climate in the Riverside district." },
    { title: "Old Town climate team",  count: "69",  thumb: "img/climate-oldtown.webp",   thumbAlt: "A green, tree-shaded path in the Old Town district",   desc: "Wanted: your idea for a better climate in the Old Town district." },
    { title: "Harbour climate team",   count: "132", thumb: "img/climate-harbour.webp",   thumbAlt: "A leafy green park in the Harbour district",          desc: "Wanted: your idea for a better climate in the Harbour district." },
    { title: "Northgate climate team", count: "98",  thumb: "img/climate-northgate.webp", thumbAlt: "An autumn tree-lined avenue in the Northgate district",desc: "Wanted: your idea for a better climate in the Northgate district." },
    { title: "Westside climate team",  count: "211", thumb: "img/climate-westside.webp",  thumbAlt: "A green residential street lined with trees in the Westside district", desc: "Wanted: your idea for a better climate in the Westside district." },
    { title: "Lakeshore climate team", count: "54",  thumb: "img/climate-lakeshore.webp", thumbAlt: "A sunny, tree-lined street in the Lakeshore district", desc: "Wanted: your idea for a better climate in the Lakeshore district." }
  ];
  var cards = props.cards !== undefined ? props.cards : DEFAULT;
  var html = cards.map(function (c) {
    var href = c.href != null ? c.href : "#";
    var label = (c.count != null ? c.count : "") + " participants";
    return (
      '<article class="gv-pcard light climate-team">' +
        '<div class="gv-pcard__thumb"><img src="' + esc(c.thumb || "") + '" alt="' + esc(c.thumbAlt || "") + '" loading="lazy" /></div>' +
        '<div class="gv-pcard__body">' +
          '<h3 class="gv-pcard__title"><a href="' + esc(href) + '">' + esc(c.title || "") + '</a></h3>' +
          '<div class="gv-bubbles xs" aria-label="' + esc(label) + '">' +
            '<span class="av"></span><span class="av"></span><span class="av"></span>' +
            '<span class="gv-bubbles__label">' + esc(label) + '</span>' +
          '</div>' +
          '<p class="gv-pcard__desc">' + esc(c.desc || "") + '</p>' +
        '</div>' +
      '</article>'
    );
  }).join("");
  var title = props.title != null ? props.title : "City climate teams";
  var hid = props.headingId != null ? props.headingId : "teams-title";
  var aria = props.ariaLabel != null ? props.ariaLabel : (title + ", scroll for more");
  return '<section class="gv-section" aria-labelledby="' + esc(hid) + '">' +
      '<div class="gv-section__head"><h2 class="gv-title h2" id="' + esc(hid) + '">' + esc(title) + '</h2></div>' +
      '<div class="gv-section__body"><div class="gv-rail" tabindex="0" role="group" aria-label="' + esc(aria) + '">' + html + '</div></div>' +
    '</section>';
});

/* ── events / call-to-action ────────────────────────────────────────────────
 * Two more homepage-builder widgets as lockable masters (mirror GVWidgets.homepage
 * 'events' + 'call-to-action'). 'events' = a grid of bordered event cards (header
 * image + date chip, clock/link-or-location/registrants rows, Register); 'call-to-action'
 * = the proposals band. Same coexistence pattern as the other list widgets. The event
 * cards use a .gv-events__grid (no .gv-rail) and data-gv-icon glyphs (filled in
 * GV.hydrate before the hash is stamped), so they stay LINKED. */
GV.register("events", function (props) {
  props = props || {};
  var esc = GV.esc;
  function eventCard(e) {
    var rsvp = e.rsvp ? '<span class="gv-event-card__rsvp">Going</span>' : '';
    var loc = e.link
      ? '<p class="gv-event-card__row"><span data-gv-icon="link" aria-hidden="true"></span> <a href="#">' + esc(e.location || '') + '</a></p>'
      : '<p class="gv-event-card__row"><span data-gv-icon="location-simple" aria-hidden="true"></span> ' + esc(e.location || '') + '</p>';
    return '<article class="gv-event-card bordered">' +
        '<div class="gv-event-card__media"><img src="' + esc(e.thumb || '') + '" alt="' + esc(e.thumbAlt || '') + '" loading="lazy" />' +
          '<span class="gv-event-card__date"><span class="m">' + esc(e.month || '') + '</span><span class="d">' + esc(e.day || '') + '</span><span class="y">' + esc(e.year != null ? e.year : '2026') + '</span></span>' + rsvp +
        '</div>' +
        '<div class="gv-event-card__body"><h3 class="gv-event-card__title"><a href="#">' + esc(e.title || '') + '</a></h3>' +
          '<div class="gv-event-card__meta">' +
            '<p class="gv-event-card__row"><span data-gv-icon="clock" aria-hidden="true"></span> ' + esc(e.when || '') + '</p>' + loc +
            '<p class="gv-event-card__row"><span data-gv-icon="user" aria-hidden="true"></span> ' + esc(e.registrants || '') + ' registrants</p>' +
          '</div>' +
          '<a class="gv-btn primary full" href="#">' + esc(e.btnLabel != null ? e.btnLabel : 'Register') + '</a>' +
        '</div></article>';
  }
  var title = props.title != null ? props.title : "Upcoming and ongoing events";
  var hid = props.headingId != null ? props.headingId : "events-title";
  var viewAll = props.viewAllLabel != null ? props.viewAllLabel : "View all events";
  var DEFAULT = [
    { thumb: "img/event-online.webp", thumbAlt: "Residents and staff at an online-and-in-person community session", month: "Jun", day: "18", title: "Community session: bringing online & offline participation together", when: "18 Jun 2026 · 11:00 – 12:00", location: "Online meeting", link: 1, registrants: "2", rsvp: 1 },
    { thumb: "img/event-walk.webp", thumbAlt: "Residents gathered on Station Road to view the plans on site", month: "Jun", day: "25", title: "Station Road walk & talk: see the plans on site", when: "25 Jun 2026 · 17:30 – 19:00", location: "Northgate station, Station Road", registrants: "41" },
    { thumb: "img/event-budget.webp", thumbAlt: "Residents asking questions at a participatory budget session", month: "Jul", day: "02", title: "Participatory budget: info & Q&A session", when: "2 Jul 2026 · 18:00 – 19:30", location: "Online meeting", link: 1, registrants: "7" }
  ];
  var cards = props.events !== undefined ? props.events : DEFAULT;
  return '<section class="gv-section" aria-labelledby="' + esc(hid) + '">' +
      '<div class="gv-section__body">' +
        '<div class="hb-events__head"><h2 class="gv-title h2" id="' + esc(hid) + '" style="margin:0">' + esc(title) + '</h2>' +
          '<a class="gv-btn text" href="#">' + esc(viewAll) + '</a></div>' +
        '<div class="gv-events__grid">' + cards.map(eventCard).join('') + '</div>' +
      '</div></section>';
});

GV.register("call-to-action", function (props) {
  props = props || {};
  var esc = GV.esc;
  var hid = props.headingId != null ? props.headingId : "cta-title";
  var title = props.title != null ? props.title : "What is your proposal?";
  var lead = props.lead != null ? props.lead : "Post your proposal on this platform, gather support and place it on the city’s agenda.";
  var ctaLabel = props.ctaLabel != null ? props.ctaLabel : "Explore all proposals";
  var href = props.ctaHref != null ? props.ctaHref : "#";
  return '<section class="gv-ctaband" aria-labelledby="' + esc(hid) + '">' +
      '<div class="gv-ctaband__inner">' +
        '<h2 class="gv-ctaband__title" id="' + esc(hid) + '">' + esc(title) + '</h2>' +
        '<p class="gv-ctaband__lead">' + esc(lead) + '</p>' +
        '<a class="gv-btn primary size-m" href="' + esc(href) + '">' + esc(ctaLabel) + '</a>' +
      '</div></section>';
});
 ...
 * On a successful mount the host is stamped data-gv-rendered (provenance) +
 * data-gv-hash (so tooling can spot a post-mount hand-edit); a render that throws is
 * left unstamped. ── */
GV.mountAll = function (root) {
  ...
  Array.prototype.forEach.call(nodes, function (el) {
    ...
    try {
      el.innerHTML = GV.render(name, props); el.__gvMounted = 1; GV.hydrate(el);
      el.setAttribute("data-gv-rendered", name);
      el.setAttribute("data-gv-hash", GV.hash(el.innerHTML));
    }
    catch (e) { if (window.console) console.warn("GV.mountAll: " + e.message, el); }
  });
  return root;
};
/* Re-hash every rendered host so its data-gv-hash matches the CURRENT DOM. Late, async
 * hydrators (theme logos via MutationObserver, avatar faces, a theme switch) legitimately
 * mutate a stamped subtree AFTER mountAll took its hash — without this they'd read as
 * "modified" in the overlay. The fillers call this once they've finished; a genuine
 * hand-edit (which never calls it) still shows as modified, so the honesty signal holds. */
GV.restampAll = function (root) {
  root = (root && root.querySelectorAll) ? root : document;
  Array.prototype.forEach.call(root.querySelectorAll("[data-gv-rendered]"), function (h) {
    try { h.setAttribute("data-gv-hash", GV.hash(h.innerHTML)); } catch (e) {}
  });
};
/* ════════════════════════════════════════════════════════════════════════════
   CSS-only utility primitives — promoted to first-class GV instances so they live
   in the single registry (and the generated recall layer), not in prose. Markup
   transcribed from the former components.md primitive sections; the .gv-* styling
   is canonical in govocal-ui.css / govocal-primitives.css.
   ════════════════════════════════════════════════════════════════════════════ */

/* chip — small rounded-pill tag for inline metadata (location, category, count). */
GV.register("chip", function (props) {
  props = props || {}; var esc = GV.esc;
  var label = props.label !== undefined ? props.label : "District 4";
  var icon  = props.icon  !== undefined ? props.icon  : "location-simple";
  return '<span class="gv-chip">' + (icon ? '<span data-gv-icon="' + esc(icon) + '" aria-hidden="true"></span> ' : '') + esc(label) + '</span>';
});

/* spinner — loading indicator (default 32px, .sm = 20px). */
GV.register("spinner", function (props) {
  props = props || {};
  return '<span class="gv-spinner' + (props.size === "sm" ? " sm" : "") + '"></span>';
});

/* progress — generic tenant-tinted track + fill (time-remaining, thresholds, completion). */
GV.register("progress", function (props) {
  props = props || {};
  var v = props.value !== undefined ? props.value : 18;
  var label = props.label !== undefined ? props.label : "Time elapsed";
  return '<div class="gv-progress" role="progressbar" aria-valuenow="' + v + '" aria-valuemin="0" aria-valuemax="100" aria-label="' + GV.esc(label) + '">' +
    '<div class="gv-progress__fill" style="width:' + v + '%"></div></div>';
});

/* tabs — underline tab strip with optional count (homepage/projects toolbars). */
GV.register("tabs", function (props) {
  props = props || {}; var esc = GV.esc;
  var tabs = props.tabs || [{ label: "Published", count: "21", selected: true }, { label: "Archived", count: "1" }];
  return '<div class="gv-tabs" role="tablist" aria-label="' + esc(props.label || "Publication status") + '">' +
    tabs.map(function (t) {
      return '<button class="gv-tab" role="tab" aria-selected="' + (t.selected ? "true" : "false") + '">' + esc(t.label) +
        (t.count != null ? ' <span class="ct">(' + esc(t.count) + ')</span>' : '') + '</button>';
    }).join('') + '</div>';
});

/* filter — the two filter-trigger atoms: neutral toolbar button + tenant-primary pill. */
GV.register("filter", function (props) {
  props = props || {}; var esc = GV.esc;
  return '<button class="gv-filter-btn">' + esc(props.btnLabel || "All topics") + '</button> ' +
    '<button class="gv-filter-pill" aria-haspopup="listbox" aria-expanded="false" aria-label="' + esc(props.pillLabel || "Date") + '">' + esc(props.pillLabel || "Date") + '</button>';
});

/* segmented — view switchers: grey-track .gv-viewseg + white-wrapper .gv-pviewtoggle. */
GV.register("segmented", function (props) {
  props = props || {}; var esc = GV.esc;
  return '<div class="gv-viewseg" role="tablist" aria-label="View ideas as">' +
      '<button class="gv-viewseg__tab" role="tab" aria-selected="true"><span data-gv-icon="menu" aria-hidden="true"></span>List</button>' +
      '<button class="gv-viewseg__tab" role="tab" aria-selected="false"><span data-gv-icon="map" aria-hidden="true"></span>Map</button>' +
    '</div>' +
    '<div class="gv-pviewtoggle" role="tablist" aria-label="View as">' +
      '<button class="gv-pviewtoggle__tab" role="tab" aria-selected="true">List</button>' +
      '<button class="gv-pviewtoggle__tab" role="tab" aria-selected="false">Perspective</button>' +
    '</div>';
});

/* searchfield — full-width search wrapper: a .gv-input with a right-aligned magnifier. */
GV.register("searchfield", function (props) {
  props = props || {}; var esc = GV.esc;
  var ph = props.placeholder !== undefined ? props.placeholder : "Search projects and folders";
  return '<div class="gv-searchfield">' +
    '<label class="gv-sr-only" for="q">Search</label>' +
    '<input class="gv-input" id="q" type="search" placeholder="' + esc(ph) + '" />' +
    '<span class="gv-searchfield__icon" aria-hidden="true"><span data-gv-icon="search"></span></span></div>';
});

/* select — native <select> wearing the .gv-input style + chevron; wrap in .gv-select. */
GV.register("select", function (props) {
  props = props || {}; var esc = GV.esc;
  var opts = props.options || ["Most recent", "Most discussed"];
  return '<div class="gv-select"><select aria-label="' + esc(props.label || "Sort by") + '">' +
    '<option class="placeholder" value="" selected disabled hidden>' + esc(props.placeholder || "Choose…") + '</option>' +
    opts.map(function (o) { return '<option>' + esc(o) + '</option>'; }).join('') + '</select></div>';
});

/* table — .gv-table; inner borders opt-in (.borders-body-rows/-cells/-header-cells). */
GV.register("table", function (props) {
  props = props || {}; var esc = GV.esc;
  var cols = props.cols || ["Name", "Status"];
  var rows = props.rows || [["Bike lane consultation", "Open"], ["Park redesign", "Closed"]];
  return '<table class="gv-table ' + esc(props.variant || "borders-body-rows") + '">' +
    '<thead><tr>' + cols.map(function (c, i) { return '<th' + (i === 0 ? ' class="clickable"' : '') + '>' + esc(c) + '</th>'; }).join('') + '</tr></thead>' +
    '<tbody>' + rows.map(function (r) { return '<tr>' + r.map(function (c) { return '<td>' + esc(c) + '</td>'; }).join('') + '</tr>'; }).join('') + '</tbody></table>';
});

/* tooltip — CSS-only light-theme bubble; shows on hover/focus of the wrapper. */
GV.register("tooltip", function (props) {
  props = props || {}; var esc = GV.esc;
  return '<span class="gv-tooltip" tabindex="0">' +
    '<span data-gv-icon="' + esc(props.icon || "info") + '"></span>' +
    '<span class="gv-tooltip__bubble" role="tooltip">' + esc(props.text || "Only admins can see this field.") + '</span></span>';
});
...
/* ════════════════════════════════════════════════════════════════════════════
   method-picker — the participation-method chooser (new-project / phase Setup).
   ONE source of truth for the 7 method cards so the project editor and every
   bo-method-* page reference it instead of hand-copying 7 cards each.
   GV.render("method-picker", { selected: "collect", assetBase })
     selected  — mkey of the pre-selected card (default none)
     assetBase — path to the method illustration PNGs (default ../../ for pages/<slug>/)
   Real product treatment: methods with shipped artwork render the illustration
   (.gv-bo-methodcard__img, greyscale until hover/selected — styled in govocal-bo.css);
   `survey` has no artwork and falls back to its glyph. Markup matches the page-local
   grid it replaces, so branching JS that queries .gv-bo-methodcard keeps working.
   ════════════════════════════════════════════════════════════════════════════ */
GV.register("method-picker", function (props) {
  props = props || {};
  var esc = GV.esc;
  var sel = props.selected || "";
  var base = props.assetBase || "../../skills/govocal-ui/img/methods/";
  var METHODS = [
    ["collect", "comments", "Collect input and feedback in public", "Tap into your users' collective intelligence. Invite them to submit, discuss ideas, and/or provide feedback in a public forum.", false, "ideation"],
    ["proposals", "vote-up", "Proposals, petitions or initiatives", "Allow participants to upload ideas with a time and vote limit.", false, "proposals"],
    ["commonground", "group", "Find common ground", "Help participants surface agreement and disagreement, one idea at a time.", true, "common_ground"],
    ["survey", "survey", "Create a survey", "Understand your users' needs and thinking via a wide range of private question types.", false, "survey"],
    ["voting", "trend-up", "Conduct a voting or prioritization exercise", "Select a voting method, and have users prioritise between a few different options.", false, "voting"],
    ["information", "info-outline", "Share information", "Provide information to users, visualise results from other phases and create data rich reports.", false, "information"],
    ["volunteering", "volunteer", "Recruit participants or volunteers", "Ask users to volunteer for activities and causes or find participants for a panel.", false, "volunteering"]
  ];
  return METHODS.map(function (m) {
    var key = m[0], icon = m[1], title = m[2], desc = m[3], beta = m[4], img = m[5];
    var betaPill = beta ? '<span class="gv-bo-typebadge is-type" style="position:absolute;top:8px;left:8px">BETA</span>' : "";
    var art = img
      ? '<img class="gv-bo-methodcard__img" src="' + esc(base) + esc(img) + '.png" alt="" loading="lazy">'
      : '<span class="gv-bo-methodcard__icon" data-gv-icon="' + esc(icon) + '"></span>';
    return '<button class="gv-bo-methodcard' + (key === sel ? " is-selected" : "") + '" data-mkey="' + esc(key) + '">' +
      '<span class="gv-bo-methodcard__art">' + betaPill + art + '</span>' +
      '<span class="gv-bo-methodcard__title">' + esc(title) + '</span>' +
      '<span class="gv-bo-methodcard__desc">' + esc(desc) + '</span>' +
    '</button>';
  }).join("");
});

/* ════════════════════════════════════════════════════════════════════════════
   authcard-grid — the 3-card authentication chooser inside the access-rights
   accordions ("Who can submit / comment / react": None / Email confirmation /
   Account creation). Repeated 9-19x per bo-method-* page and in
   bo-project-phase. ONE source of truth for the grid so every access accordion
   references it instead of hand-copying the 3 cards.
   GV.render("authcard-grid", { selected: "none|email|account", cards })
     selected — key of the pre-selected card (default "account", matching the
                live default where Account creation is selected)
     cards    — optional override array of {key,icon,title,desc,badge}; defaults
                to the canonical None / Email confirmation / Account creation set.
   Markup matches the page-local .pp-authgrid it replaces (promoted to
   .gv-bo-authgrid/.gv-bo-authcard in govocal-bo.css), so the branching JS that
   queries the cards keeps working after the class swap.
   ════════════════════════════════════════════════════════════════════════════ */
GV.register("authcard-grid", function (props) {
  props = props || {};
  var esc = GV.esc;
  var sel = props.selected || "account";
  var cards = props.cards || [
    { key: "none", icon: "user", title: "None", badge: "NEW", desc: "Anyone can participate without signing up or logging in." },
    { key: "email", icon: "email", title: "Email confirmation", desc: "Participants need to confirm their email with a one-time code." },
    { key: "account", icon: "user", title: "Account creation", desc: "Participants need to create a full account with their name, confirmed email and password." }
  ];
  var cardsHtml = cards.map(function (c) {
    var badge = c.badge ? ' <span class="gv-bo-typebadge is-type">' + esc(c.badge) + '</span>' : "";
    return '<button class="gv-bo-authcard' + (c.key === sel ? " is-selected" : "") + '" data-authkey="' + esc(c.key) + '">' +
      '<span class="gv-bo-authcard__icon gv-icon" data-gv-icon="' + esc(c.icon) + '"></span>' +
      '<span class="gv-bo-authcard__title">' + esc(c.title) + badge + '</span>' +
      '<span class="gv-bo-authcard__desc">' + esc(c.desc) + '</span>' +
    '</button>';
  }).join("");
  return '<div class="gv-bo-authgrid">' + cardsHtml + '</div>';
});

/* ════════════════════════════════════════════════════════════════════════════
   multiloc-field — the label + per-field EN/ES-ES locale switcher + input cluster.
   ONE source of truth for the translatable TEXT field that recurs 10-15x per page
   across phase-name / title / description fields on bo-method-*, bo-content-builder,
   bo-project-new and bo-settings. Replaces the hand-copied
     <div class="gv-bo-field"><div class="gv-bo-field__head">
       <span class="gv-label">…</span><div class="gv-bo-multiloc">…</div></div>
       <input class="gv-input"></div>
   block so every consuming page references it instead of re-typing the cluster.

   GV.render("multiloc-field") → translatable BO text field: label + EN/ES-ES locale switcher + input/textarea.
   GV.render("multiloc-field", { label, name, value, locales, type, help, selected, values })
     label    — field label text (omit → switcher-only head, .is-switcher-only)
     name     — field name; also seeds the input id for the real <label for> link
     value    — value for the SELECTED locale (back-compat single-value shorthand)
     values   — optional { "<locale>": "<value>" } map; overrides `value` per-locale
                and drives the done/miss status dot (content present → .done)
     locales  — locale codes, e.g. ["en","es-ES"] (default); first = selected unless
                `selected` is given. Codes upper-case via CSS (en → EN).
     selected — locale code that is active (default: first in `locales`)
     type     — "text" (default) | "textarea"
     help     — optional .gv-bo-help line under the input

   A11Y FIXES baked in (the two gaps in the live markup):
   1. the label is emitted as a REAL <label for="<id>"> tied to the input id — not a
      bare <span class="gv-label"> (CSS targets the class, so zero visual change);
   2. the done/miss locale state, which was colour-only (a 9px dot), now carries a
      visually-hidden ".gv-sr-only" status phrase ("translated" / "translation
      missing") so assistive tech announces it.
   Markup otherwise matches the page-local cluster 1:1 (.gv-bo-field /
   .gv-bo-field__head / .gv-bo-multiloc / .gv-input — all already canonical in
   govocal-bo.css), so existing per-field switcher JS keeps working.
   ════════════════════════════════════════════════════════════════════════════ */
var GV_MULTILOC_UID = 0;
GV.register("multiloc-field", function (props) {
  props = props || {};
  var esc = GV.esc;
  var label = props.label;
  var name = props.name || ("multiloc-" + (++GV_MULTILOC_UID));
  var id = "f-" + String(name).replace(/[^a-zA-Z0-9_-]/g, "-") + "-" + (++GV_MULTILOC_UID);
  var locales = props.locales && props.locales.length ? props.locales : ["en", "es-ES"];
  var selected = props.selected || locales[0];
  var values = props.values || null;
  var type = props.type === "textarea" ? "textarea" : "text";

  // value resolution: explicit per-locale map wins; else single `value` applies to
  // the selected locale only (so its dot is .done and the others .miss).
  function valueFor(loc) {
    if (values && Object.prototype.hasOwnProperty.call(values, loc)) return values[loc];
    if (loc === selected) return props.value != null ? props.value : "";
    return "";
  }
  var selectedValue = valueFor(selected);

  var switcher = '<div class="gv-bo-multiloc" role="group" aria-label="Translations for ' +
    esc(label || name) + '">' +
    locales.map(function (loc) {
      var has = String(valueFor(loc) || "").length > 0;
      var state = has ? "done" : "miss";
      var statusText = has ? "translated" : "translation missing";
      return '<button type="button"' + (loc === selected ? ' class="is-active" aria-pressed="true"' : ' aria-pressed="false"') + '>' +
        '<span class="' + state + '" aria-hidden="true"></span>' +
        esc(loc) +
        '<span class="gv-sr-only"> (' + statusText + ')</span>' +
      '</button>';
    }).join("") +
    '</div>';

  var labelHtml = label != null && label !== ""
    ? '<label class="gv-label" for="' + esc(id) + '">' + esc(label) + '</label>'
    : "";
  var head = '<div class="gv-bo-field__head' + (labelHtml ? "" : " is-switcher-only") + '">' +
    labelHtml + switcher + '</div>';

  var control = type === "textarea"
    ? '<textarea class="gv-input" id="' + esc(id) + '" name="' + esc(name) + '" rows="5" style="resize:vertical">' + esc(selectedValue) + '</textarea>'
    : '<input class="gv-input" id="' + esc(id) + '" name="' + esc(name) + '" type="text" value="' + esc(selectedValue) + '" />';

  var help = props.help ? '<p class="gv-bo-help">' + esc(props.help) + '</p>' : "";

  return '<div class="gv-bo-field">' + head + control + help + '</div>';
});

/* ════════════════════════════════════════════════════════════════════════════
   phase-setup-form — the "Edit Phase" setup scaffold repeated ~8x per bo-method-*
   page (one .pp-setup variant per participation method). Each panel shares the
   SAME top scaffold — formhead ("Edit Phase") + a phase-name multiloc-field +
   a Dates dateface + the "What do you want to do?" method-picker + a trailing
   Save — and differs ONLY by which method is pre-selected and by a method-specific
   BODY. ONE source of truth for that scaffold so each page references it instead
   of hand-copying the cluster 8 times.

   GV.render("phase-setup-form") → "Edit Phase" setup scaffold: phase-name + dates + method-picker + save, with a method-specific body slot.
   GV.render("phase-setup-form", { method, sub, active, name, dates, pickerLede, intro, body })
     method     — mkey of the pre-selected method card (e.g. "collect"); also the
                  default for `sub` and the picker's data-method.
     sub        — data-sub value for the .pp-setup panel (default: method). The
                  page's sub-tab/branch JS targets the panel by this value.
     active     — true → the panel renders with .is-active (the default Setup view).
     name       — props for the phase-name multiloc-field: { label, name, value,
                  type }. Emitted as a [data-multiloc] mount so the page's existing
                  multiloc hydration owns it (1 source of truth, JS untouched).
     dates      — { start, end } for the Dates dateface (end default "Open ended").
     pickerLede — helper line under "What do you want to do?" (default canonical).
     intro      — optional HTML string slot rendered between the Dates field and the
                  shared pre-picker Save (the collect variant's info banner lives here).
     body       — HTML string slot rendered AFTER the picker (the method-specific
                  config). The page keeps its body markup in a JS template/const,
                  preserving every existing id/hook/JS reference verbatim.

   BODY-SLOT APPROACH (Option A — body as an HTML string): the method-specific body
   is large, varies per method and carries its own ids/JS hooks; abstracting it
   would be high-risk. So the component owns only the REPEATED scaffold and accepts
   the body as an opaque HTML string — the page keeps that markup 1:1, so all its
   wiring (branch/switch/save/validation) keeps referencing the same elements.

   Markup matches the page-local .pp-setup panel 1:1 (.gv-bo-formhead, .gv-bo-field,
   .gv-bo-dateface, .gv-bo-subhead, .pp-lede, .gv-bo-methods.pp-methodgrid, the
   .gv-btn admin-dark Save) — all already canonical — and emits the same
   [data-multiloc] / .pp-methodgrid mount points the page already hydrates, so the
   branch logic that queries .pp-setup / .pp-picker / the Save buttons keeps working.
   ════════════════════════════════════════════════════════════════════════════ */
GV.register("phase-setup-form", function (props) {
  props = props || {};
  var esc = GV.esc;
  var method = props.method || "";
  var sub = props.sub != null ? props.sub : method;
  var name = props.name || {};
  var dates = props.dates || {};
  var startDate = dates.start != null ? dates.start : "";
  var endDate = dates.end != null ? dates.end : "Open ended";
  var lede = props.pickerLede != null ? props.pickerLede
    : "Each phase runs exactly one participation method.";

  // phase-name: emit a [data-multiloc] mount so the page's existing
  // GV.render("multiloc-field") hydration renders it (single source of truth; no
  // double-render). The mount declares the props the page's multiloc loop reads.
  var nameMount = '<div data-multiloc' +
    (name.label != null ? ' data-label="' + esc(name.label) + '"' : '') +
    (name.name != null ? ' data-name="' + esc(name.name) + '"' : '') +
    (name.type != null ? ' data-type="' + esc(name.type) + '"' : '') +
    (name.value != null ? ' data-value="' + esc(name.value) + '"' : '') +
  '></div>';

  var datesField = '<div class="gv-bo-field">' +
    '<span class="gv-label">Dates</span>' +
    '<div class="gv-bo-dateface"><span>' + esc(startDate) + '</span>' +
    '<span class="sep">›</span><span>' + esc(endDate) + '</span>' +
    '<span class="gv-icon" data-gv-icon="calendar"></span></div>' +
  '</div>';

  // intro slot (e.g. the collect variant's pre-picker info banner) — raw HTML,
  // optional, rendered between the Dates field and the shared pre-picker Save.
  var intro = props.intro || "";

  // pre-picker Save — part of the shared scaffold (present in every variant); the
  // page's save/dirty JS wires BOTH this and the trailing Save by /save/ text.
  var prePickerSave = '<button class="gv-btn admin-dark" disabled style="margin-bottom:8px">' +
    '<span class="gv-btn__label">Save changes</span></button>';

  // the method picker: emit the same .pp-methodgrid mount the page hydrates with
  // GV.render("method-picker", { selected: data-method }); branch JS owns selection.
  var pickerBlock = '<h4 class="gv-bo-subhead">What do you want to do?</h4>' +
    '<p class="pp-lede">' + esc(lede) + '</p>' +
    '<div class="gv-bo-methods pp-methodgrid" data-method="' + esc(method) + '"></div>';

  // body slot (method-specific) — raw HTML string, kept verbatim by the page.
  var body = props.body || "";

  var trailingSave = '<div style="margin-top:28px"><button class="gv-btn admin-dark" disabled>' +
    '<span class="gv-btn__label">Save changes</span></button></div>';

  return '<div class="pp-setup pp-sub' + (props.active ? ' is-active' : '') + '" data-sub="' + esc(sub) + '">' +
    '<h3 class="gv-bo-formhead">Edit Phase</h3>' +
    nameMount +
    datesField +
    intro +
    prePickerSave +
...
/* ════════════════════════════════════════════════════════════════════════════
   importcard-pair — the two-card input-import block (.gv-bo-importlist wrapping
   two .gv-bo-importcard rows: Paper forms OCR + Spreadsheet). Each card is an
   icon tile + title + description + a row of right-aligned actions. ONE source of
   truth for the block, which is duplicated VERBATIM between the Input importer
   tab and the Input form tab on bo-method-budgeting / -ideation / -information /
   -voting (and the other bo-method-* pages) so every consuming tab references it
   instead of hand-copying the two cards.

   GV.render("importcard-pair") → canonical Paper forms OCR + Spreadsheet import pair.
   GV.render("importcard-pair", { cards })
     cards — array of { icon, title, desc, actions }; defaults to the canonical
             Paper-forms-OCR + Spreadsheet pair (the Input-form-tab variant).
             Each action is { type, icon, label, infodot, href }:
               type    — "link" (default, → .gv-bo-link) | "button"
                         (→ .gv-btn admin-dark size-s, the primary "Import scans")
               icon    — leading data-gv-icon (default "download")
               label   — visible text
               infodot — true → trailing .pp-infodot info-outline glyph (Template)
               href    — link href (default "#"; ignored for buttons)

   Markup matches the page-local block 1:1 — .gv-bo-importlist / .gv-bo-importcard
   / __icon / __main / __title / __desc / __actions, plus .gv-bo-link and
   .gv-btn admin-dark size-s — all already canonical in govocal-bo.css, so there
   is ZERO visual change and any JS that queries .gv-bo-importcard keeps working.
   ════════════════════════════════════════════════════════════════════════════ */
GV.register("importcard-pair", function (props) {
  props = props || {};
  var esc = GV.esc;
  var cards = props.cards || [
    {
      icon: "survey",
      title: "Paper forms OCR",
      desc: "Print form, collect on paper, scan responses back",
      actions: [
        { type: "link", icon: "download", label: "Download PDF" },
        { type: "button", icon: "open-in-new", label: "Import scans" }
      ]
    },
    {
      icon: "menu",
      title: "Spreadsheet",
      desc: "Download a template, fill it in, and upload responses",
      actions: [
        { type: "link", icon: "download", label: "Template" },
        { type: "link", icon: "open-in-new", label: "Import file" }
      ]
    }
  ];
  ...
  function renderAction(a) {
    a = a || {};
    var icon = a.icon != null ? a.icon : "download";
    var iconHtml = icon ? '<span data-gv-icon="' + esc(icon) + '"></span>' : "";
    if (a.type === "button") {
      return '<button class="gv-btn admin-dark size-s">' + iconHtml +
        '<span class="gv-btn__label">' + esc(a.label || "") + '</span></button>';
    }
    var dot = a.infodot
      ? ' <span class="gv-icon pp-infodot" data-gv-icon="info-outline"></span>'
      : "";
    return '<a class="gv-bo-link" href="' + esc(a.href != null ? a.href : "#") + '">' +
      iconHtml + esc(a.label || "") + dot + '</a>';
  }

  var cardsHtml = cards.map(function (c) {
    c = c || {};
    var actions = (c.actions || []).map(renderAction).join("");
    return '<div class="gv-bo-importcard">' +
      '<span class="gv-bo-importcard__icon"><span class="gv-icon" data-gv-icon="' + esc(c.icon || "survey") + '"></span></span>' +
      '<div class="gv-bo-importcard__main">' +
        '<div class="gv-bo-importcard__title">' + esc(c.title || "") + '</div>' +
        '<div class="gv-bo-importcard__desc">' + esc(c.desc || "") + '</div>' +
      '</div>' +
      '<div class="gv-bo-importcard__actions">' + actions + '</div>' +
    '</div>';
  }).join("");

  return '<div class="gv-bo-importlist">' + cardsHtml + '</div>';
});

/* ════════════════════════════════════════════════════════════════════════════
   statline — the back-office stat block: a label + a big number + an optional
   sub / delta line. ONE source of truth for the three page-local variants that
   all express "labelled metric":

     variant "card"  (default) — the .gv-bo-stat insight card (label + trailing
                       icon + big blue number + a muted delta line). ~6× in the
                       STATS loop on bo-project-phase and the bo-dashboard
                       Insights grid (.gv-bo-statgrid host).
     variant "inline" — a bare .db-stat item for the .db-statline run inside a
                       Dashboard card body (cardHead bodies on bo-dashboard:
                       Events / Invites / Team). label is the blue __top label;
                       sub is the muted "Last 30 days: …" line.
     variant "audience" — an .aud-stat tile (sits inside a .gv-bo-chartcard) for
                       the Traffic stat cards on bo-method-* (Visitors / Visits /
                       Visit duration …). icon → trailing info-outline glyph.

   GV.render("statline", { label, number, sub, icon, variant, delta, infodot })
     label   — the metric label (required-ish; escaped).
     number  — the big number / value (escaped). Alias: `num`.
     sub     — optional sub line ("Last 30 days: 412"); rendered as the delta
               (card) / __lbl (inline) / __sub (audience). HTML allowed via
               `subHtml` for the <b>…</b> emphasis the originals use.
     icon    — data-gv-icon name. Card: trailing __icon. Audience: trailing
               info-outline-style glyph after the label (pass icon:"info-outline").
     variant — "card" (default) | "inline" | "audience".
     delta   — card-only override for the bottom line when there is no `sub`
               (default "<b>0%</b> 7-day change", matching the STATS loop).
     infodot — audience-only convenience: true → trailing info-outline glyph.

   Markup matches the page-local blocks 1:1 — .gv-bo-stat / __top / __label /
   __icon / __num / __delta · .db-stat__top / __lbl / __num · .aud-stat / __lbl /
   __num / __sub — all canonical in govocal-bo.css, so ZERO visual change and any
   JS that queries these classes keeps working.
   ════════════════════════════════════════════════════════════════════════════ */
GV.register("statline", function (props) {
  props = props || {};
  var esc = GV.esc;
  var variant = props.variant || "card";
  var label = props.label != null ? props.label : "";
  var number = props.number != null ? props.number : (props.num != null ? props.num : "");
  // sub line: prefer raw `subHtml` (for the <b> emphasis), else escaped `sub`.
  var subHtml = props.subHtml != null ? props.subHtml : (props.sub != null ? esc(props.sub) : "");
  var icon = props.icon;

  function iconGlyph(extraClass) {
    if (!icon) return "";
    return '<span class="' + extraClass + '" data-gv-icon="' + esc(icon) + '"></span>';
  }

  if (variant === "inline") {
    return '<div>' +
      '<div class="db-stat__lbl db-stat__top">' + esc(label) + '</div>' +
      '<div class="db-stat__num">' + esc(number) + '</div>' +
      (subHtml ? '<div class="db-stat__lbl">' + subHtml + '</div>' : '') +
    '</div>';
  }

  if (variant === "audience") {
    var dot = (icon || props.infodot)
      ? ' <span class="gv-icon" data-gv-icon="' + esc(icon || "info-outline") + '"></span>'
      : '';
    return '<div class="gv-bo-chartcard aud-stat">' +
      '<span class="aud-stat__lbl">' + esc(label) + dot + '</span>' +
      '<span class="aud-stat__num">' + esc(number) + '</span>' +
      (subHtml ? '<span class="aud-stat__sub">' + subHtml + '</span>' : '') +
    '</div>';
  }

  // variant "card" (default) — .gv-bo-stat insight card.
  var deltaHtml = subHtml || (props.delta != null ? props.delta : '<b>0%</b> 7-day change');
  return '<div class="gv-bo-stat">' +
    '<div class="gv-bo-stat__top">' +
      '<span class="gv-bo-stat__label">' + esc(label) + '</span>' +
      iconGlyph('gv-bo-stat__icon gv-icon') +
    '</div>' +
    '<div class="gv-bo-stat__num">' + esc(number) + '</div>' +
    '<div class="gv-bo-stat__delta">' + deltaHtml + '</div>' +
  '</div>';
});

/* ════════════════════════════════════════════════════════════════════════════
   pager — the back-office list pagination nav: a prev icon-button, a windowed
   run of numbered page buttons with leading/trailing "…" ellipses, and a next
   icon-button. ONE source of truth for the .gv-bo-pager block hand-copied at the
   foot of the BO list tables (bo-input-manager posts table, bo-method-information
   participants table, and other bo-* list pages).

   GV.render("pager", { page, total, windowSize })
     page       — the current 1-based page (default 1; clamped to 1..total).
     total      — total number of pages (default 1).
     windowSize — how many numbered buttons to show around the current page
                  before collapsing to ellipses (default 3 → "1 2 3 … 30" on
                  page 1, matching the live markup).

   Markup matches the page-local block 1:1 — <nav class="gv-bo-pager"> with a
   leading prev <button> (disabled on the first page), numbered <button>s
   (.is-active on the current page), bare <span>…</span> ellipses, and a trailing
   next <button> (disabled on the last page). All classes are canonical in
   govocal-bo.css (the muted ellipsis colour was promoted to `.gv-bo-pager span`
   so the bare span no longer needs a page-local inline style), so ZERO visual
   change and any JS that queries .gv-bo-pager button keeps working.
   ════════════════════════════════════════════════════════════════════════════ */
GV.register("pager", function (props) {
  props = props || {};
  var esc = GV.esc;
  var total = Math.max(1, parseInt(props.total, 10) || 1);
  var page = parseInt(props.page, 10) || 1;
  if (page < 1) page = 1;
  if (page > total) page = total;
  var win = Math.max(1, parseInt(props.windowSize, 10) || 3);

  // window of page numbers around the current page, always anchored so that the
  // first/last pages are reachable and collapsed to "…" when out of range.
  var half = Math.floor((win - 1) / 2);
  var start = page - half;
  var end = start + win - 1;
  if (start < 1) { start = 1; end = Math.min(total, win); }
  if (end > total) { end = total; start = Math.max(1, total - win + 1); }

  var items = [];
  // leading 1 + ellipsis when the window has drifted right.
  if (start > 1) {
    items.push(1);
  ...
  var numbersHtml = items.map(function (it) {
    if (it === "…") return '<span>…</span>';
    return '<button' + (it === page ? ' class="is-active"' : '') + '>' + esc(String(it)) + '</button>';
  }).join("");

  var prevHtml = '<button aria-label="Previous"' + (page <= 1 ? ' disabled' : '') +
    '><span data-gv-icon="chevron-left"></span></button>';
  var nextHtml = '<button aria-label="Next"' + (page >= total ? ' disabled' : '') +
    '><span data-gv-icon="chevron-right"></span></button>';

  return '<nav class="gv-bo-pager" aria-label="Pagination">' +
    prevHtml + numbersHtml + nextHtml +
  '</nav>';
});

/* ── bo-project-rail ──────────────────────────────────────────────────────────
 * bo-project-rail.js — canonical instance renderer for the Project-Editor LEFT
 * rail. GV.render("bo-project-rail", props) → canonical HTML string.
 *
 * The white 280px secondary panel that sits beside the dark global nav
 * (.gv-bo-side): the project's destinations (Project page + a collapsible Manage
 * group) and the vertical PHASE TIMELINE (status dots + dates + New phase) plus
 * the off-timeline Extras group. Extracted from parallel-editor-builder-v4.
 *
 * BO surface — fixed neutral BO palette (.gv-bo-projrail*), NEVER city-themed.
 *
 * The rail is STATELESS: the empty / few / complex variants are just different
 * `phases` / `items` DATA. It emits data-* hooks (data-view / data-phase /
 * data-action) for the host to wire — it bakes in no onclick handlers.
 *
 * Props (all optional — defaults reproduce the rich "complex" project):
 *   project  {label, view, active, chevron, heading}   top destination row
 *            (omit `heading` for the heading-less, chevron-disclosed layout)
 *   manage   {label, items:[{label, view, badge}]} | false   collapsible group
 *            (omit `label` for a heading-less group disclosed by the Project chevron)
 *   timeline {heading, intro, example, addLabel,
 *             phases:[{name, dates, status, view, past}]} | false
 *   extras   {heading, sub, addLabel,
 *             items:[{name, dates, status, view}]} | false
 *
 *   phase/extras `status`: "live" | "upcoming" | "completed" | "planned"(default)
 */

var BO_PROJRAIL_TIMELINE = {
  heading: "Timeline",
  addLabel: "New phase",
  phases: [
    { name: "Pre-survey",         dates: "1–15 May · Survey",        status: "completed", past: true, view: "presurvey" },
    { name: "Collect ideas",      dates: "20 May–10 Jun · Ideation", status: "live",                  view: "collect" },
    { name: "What we heard",      dates: "11–25 Jun · Information",   status: "upcoming",              view: "heard" },
    { name: "Community vote",     dates: "26 Jun–12 Jul · Voting",    status: "upcoming",              view: "vote" },
    { name: "Find common ground", dates: "13–27 Jul · Common ground", status: "upcoming",             view: "commonground" }
  ]
};

var BO_PROJRAIL_MANAGE = {
  // Heading-less by default: the utility group is disclosed by the Project-page
  // chevron (matches GoVocal's current newBackoffice rail, where the same items
  // fold under "Project page" with no separate "Manage" label). Pass a `label`
  // to opt back into the standalone "Manage" toggle heading.
  items: [
    { label: "Audience",  view: "audience" },
    { label: "Messaging", view: "messaging" },
    { label: "Events",    view: "events", badge: 3 },
    { label: "Input 360", view: "input360" },
    { label: "Settings",  view: "settings" }
  ]
};

var BO_PROJRAIL_EXTRAS = {
  heading: "Extras",
  sub: "Surveys that run outside your timeline",
  addLabel: "New survey",
  items: [
    { name: "Ask us anything", dates: "Ongoing", status: "live", view: "askus" }
  ]
};

GV.register("bo-project-rail", function (props) {
  props = props || {};
  var esc = GV.esc;

  // A status → dot-modifier class (planned/unknown → neutral grey, no modifier).
  function dotClass(status) {
    var s = { live: 1, upcoming: 1, completed: 1, add: 1 }[status] ? status : "";
    return "gv-bo-projrail__dot" + (s ? " gv-bo-projrail__dot--" + s : "");
  }

  // One phase / survey row. Shared by the timeline and the extras group.
  // name/dates are escaped; pass nameHtml/metaHtml for trusted rich labels
  // (e.g. a "Link only" pill or a placement tag).
  function phaseRow(p) {
    var nameCls = "gv-bo-projrail__name" + (p.past ? " gv-bo-projrail__name--past" : "");
    var nameInner = p.nameHtml != null ? p.nameHtml : esc(p.name);
    var metaInner = p.metaHtml != null ? p.metaHtml : (p.dates != null ? esc(p.dates) : "");
    var meta = metaInner ? '<div class="gv-bo-projrail__meta">' + metaInner + "</div>" : "";
    var activeCls = (p.view && p.view === props.active) ? " is-active" : "";
    return '<div class="gv-bo-projrail__phase' + activeCls + '"' + (p.view ? ' data-phase="' + esc(p.view) + '"' : "") + ">" +
      '<div class="' + dotClass(p.status) + '"></div>' +
      '<div class="gv-bo-projrail__phasebody">' +
        '<div class="' + nameCls + '">' + nameInner + "</div>" + meta +
      "</div>" +
    "</div>";
  }

  function addBtn(label, action, muted) {
    return '<button type="button" class="gv-bo-projrail__add' + (muted ? " gv-bo-projrail__add--muted" : "") + '"' +
      (action ? ' data-action="' + esc(action) + '"' : "") + ">" +
      '<span class="gv-bo-projrail__add-ic" data-gv-icon="plus" aria-hidden="true"></span>' +
      esc(label) + "</button>";
  }

  // ── Project destination(s) ──
  // `heading` is opt-in (omit it for the heading-less, chevron-disclosed layout).
  var project = props.project || { label: "Project page", view: "project", active: true, chevron: true };
  var chev = project.chevron ? '<svg class="gv-bo-projrail__chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><polyline points="6 9 12 15 18 9"></polyline></svg>' : "";
  var projHead = project.heading ? '<div class="gv-bo-projrail__head">' + esc(project.heading) + "</div>" : "";
  // Active when the row explicitly says so OR the rail's `active` view id matches it.
  var projActive = project.active || props.active === (project.view || "project");
  var projHtml = projHead +
    '<div class="gv-bo-projrail__item' + (projActive ? " is-active" : "") + '" data-view="' + esc(project.view || "project") + '">' +
      '<span class="gv-bo-projrail__label">' + esc(project.label || "Project page") + "</span>" + chev +
    "</div>";

  // ── Manage group (collapsible) ──
  var manage = props.manage === undefined ? BO_PROJRAIL_MANAGE : props.manage;
  var manageHtml = "";
  if (manage) {
    var manageItems = (manage.items || []).map(function (it) {
      var badge = (it.badge != null) ? '<span class="gv-bo-projrail__badge">' + esc(String(it.badge)) + "</span>" : "";
      var label = badge
        ? '<span class="gv-bo-projrail__label">' + esc(it.label) + "</span>" + badge
        : esc(it.label);
      var itActive = (it.view && it.view === props.active) ? " is-active" : "";
      return '<div class="gv-bo-projrail__item' + itActive + '" data-view="' + esc(it.view || "") + '">' + label + "</div>";
    }).join("\n          ");
    // A labelled group gets its own toggle heading; a heading-less group is just the
    // items (the host discloses it, e.g. via the Project row's chevron).
    var manageHead = manage.label ?
      '<div class="gv-bo-projrail__head gv-bo-projrail__head--toggle" data-action="toggle-manage">' +
        "<span>" + esc(manage.label) + "</span>" +
        '<svg class="gv-bo-projrail__chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><polyline points="6 9 12 15 18 9"></polyline></svg>' +
      "</div>" : "";
    manageHtml = manageHead +
      '<div class="gv-bo-projrail__group">\n          ' + manageItems + "\n        </div>";
  }

  // ── Timeline section ──
  var timeline = props.timeline === undefined ? BO_PROJRAIL_TIMELINE : props.timeline;
  var timelineHtml = "";
  if (timeline) {
    var exCls = timeline.example ? " gv-bo-projrail__example" : "";
    var intro = timeline.intro ? '<div class="gv-bo-projrail__example-intro">' + esc(timeline.intro) + "</div>" : "";
    var rows = (timeline.phases || []).map(phaseRow).join("\n            ");
    timelineHtml =
      '<div class="gv-bo-projrail__section' + exCls + '">' +
        '<hr class="gv-bo-projrail__divider" />' +
        '<div class="gv-bo-projrail__head">' + esc(timeline.heading || "Timeline") + "</div>" +
        intro +
        '<div class="gv-bo-projrail__timeline">\n            ' + rows + "\n          </div>" +
        addBtn(timeline.addLabel || "New phase", "add-phase", true) +
      "</div>";
  }

  // ── Extras section ──
  var extras = props.extras === undefined ? BO_PROJRAIL_EXTRAS : props.extras;
  var extrasHtml = "";
  if (extras) {
    var sub = extras.sub ? '<div class="gv-bo-projrail__sub">' + esc(extras.sub) + "</div>" : "";
    var exRows = (extras.items || []).map(phaseRow).join("\n          ");
    extrasHtml =
      '<div class="gv-bo-projrail__section">' +
        '<hr class="gv-bo-projrail__divider" />' +
        '<div class="gv-bo-projrail__head">' + esc(extras.heading || "Extras") + "</div>" +
        sub + exRows +
        addBtn(extras.addLabel || "New survey", "add-survey", true) +
      "</div>";
  }

  return (
    '<nav class="gv-bo-projrail" aria-label="Project navigation">\n        ' +
      projHtml + "\n        " +
      manageHtml + "\n        " +
      timelineHtml + "\n        " +
      extrasHtml + "\n      " +
...
/* ── bo-project-editor ────────────────────────────────────────────────────────
 * bo-project-editor.js — the canonical PROJECT-EDITOR SHELL: the single back-office
 * chrome that every project-scoped admin page shares (the bo-project-* section pages
 * + the bo-method-* phase pages). It composes, in one mount, the global nav as an
 * icon rail (sidebar), the project topbar, the canonical project rail
 * (bo-project-rail) and a self-scrolling STAGE slot for the page's own content.
 *
 * Before this, all 15 reference pages hand-rolled the shell markup AND a copy of the
 * rail's route-map + click/disclosure handlers. Those now live here ONCE. A page:
 *
 *   <div id="proj-editor"></div>
 *   <template id="tpl-stage"> …the page's centre-stage content… </template>
 *   <script>GV.mountProjectEditor('#proj-editor', { active:'audience', stage:'#tpl-stage' });</script>
 *
 * declares only WHICH rail item is current (`active`) + its stage content. Navigation,
 * the Project-page chevron disclosure and the topbar links are wired centrally, and
 * BO_EDITOR_NAV below is the single source of truth for the sibling-page routes.
 *
 * Renderer props (GV.mount(el,'bo-project-editor',props)):
 *   active    string  current rail view id → marks is-active, suppresses self-nav
 *   rail      object  optional partial override merged over the canonical story
 *                     ({project, manage, timeline, extras} — see bo-project-rail)
 *   topbar    object  optional bo-topbar props
 *   stageClass string extra class on the stage (e.g. 'is-preview' for a centred artboard)
 *   stageHtml string  raw HTML for the stage slot (usually injected by the helper)
 */
var BO_EDITOR_NAV = {
  project:      "../bo-project-phase/",
  ideation:     "../bo-method-ideation/",
  voting:       "../bo-method-voting/",
  budgeting:    "../bo-method-budgeting/",
  information:  "../bo-method-information/",
  mapping:      "../bo-method-mapping/",
  perspectives: "../bo-method-perspectives/",
  commonground: "../bo-method-common-ground/",
  volunteering: "../bo-method-volunteering/",
  survey:       "../bo-method-survey/",
  audience:     "../bo-project-audience/",
  messaging:    "../bo-project-messaging/",
  events:       "../bo-project-events/",
  input360:     "../bo-project-input360/",
  settings:     "../bo-project-settings/",
  "add-phase":  "../bo-project-phase/",
  "add-survey": "../bo-project-phase/"
};

// The shared reference project a NON-phase page shows in the rail (this project's
// ideation phase is live, a vote is queued, an information phase follows, with a
// survey running outside the timeline). A bo-method-* page overrides `timeline`
// (+ optionally `extras`) so its own method reads as the live phase.
var BO_EDITOR_STORY = {
  project: { label: "Project page", view: "project", chevron: true },
  manage: BO_PROJRAIL_MANAGE,
  timeline: {
    heading: "Timeline", addLabel: "New phase",
    phases: [
      { name: "New ideas!",          dates: "From 2 Jun 2024 · Ideation",    status: "live",     view: "ideation" },
      { name: "Vote on the options", dates: "From 14 Jun 2024 · Voting",     status: "upcoming", view: "voting" },
      { name: "Implementation",      dates: "From 14 Jun 2024 · Information", status: "upcoming", view: "information" }
    ]
  },
  extras: {
    heading: "Extras", sub: "Surveys that run outside your timeline", addLabel: "New survey",
    items: [ { name: "Community Survey", dates: "Since 6 May 2024 · Survey", status: "live", view: "survey" } ]
  }
};

GV.register("bo-project-editor", function (props) {
  props = props || {};
  // Merge the canonical story with a page's partial rail override (per section).
  var over = props.rail || {};
  var railProps = { active: props.active };
  ["project", "manage", "timeline", "extras"].forEach(function (k) {
    railProps[k] = over[k] !== undefined ? over[k] : BO_EDITOR_STORY[k];
  });
  var sidebarHtml = GV.render("sidebar", { active: "projects", rail: true });
  var topbarHtml  = GV.render("bo-topbar", props.topbar || {});
  var railHtml    = GV.render("bo-project-rail", railProps);
  var stageHtml   = props.stageHtml != null ? props.stageHtml : "";
  return (
    '<div class="gv-bo-shell is-rail">' +
      sidebarHtml +
      '<div class="gv-bo-main">' +
        '<div class="gv-bo-editor">' +
          topbarHtml +
          '<div class="gv-bo-editor__body">' +
            '<aside class="gv-bo-editor__railhost">' + railHtml + "</aside>" +
            '<main class="gv-bo-editor__stage' + (props.stageClass ? " " + GV.esc(props.stageClass) : "") + '" data-gv-slot>' + stageHtml + "</main>" +
          "</div>" +
        "</div>" +
      "</div>" +
    "</div>"
...
/* GV.mountProjectEditor(hostOrSel, opts) — mount the shell and wire it once.
 *   opts.active  string          current rail view id
 *   opts.rail    object          optional rail override (see BO_EDITOR_STORY)
 *   opts.stage   sel | el | <template>   the page's stage content, moved into the slot
 *   opts.stageClass string        extra class on the stage (e.g. 'is-preview')
 *   opts.topbar  object          optional bo-topbar props
 *   opts.onNav   fn(target,kind,e)   intercept a rail click (kind: 'action'|'phase'|
 *                                'item'); return true to handle it in-page (skip nav)
 * The rail navigation, the Project-chevron disclosure and the topbar links are the
 * one canonical copy here — pages no longer carry their own. */
GV.mountProjectEditor = function (host, opts) {
  opts = opts || {};
  if (typeof host === "string") host = document.querySelector(host);
  if (!host) return null;

  GV.mount(host, "bo-project-editor", { active: opts.active, rail: opts.rail, topbar: opts.topbar, stageClass: opts.stageClass });

  // Move the page's stage content into the slot (excluded from the provenance hash
  // by data-gv-slot), then mount any canonical instances it declares + fill icons.
  var stage = host.querySelector(".gv-bo-editor__stage");
  if (stage && opts.stage) {
    var src = typeof opts.stage === "string" ? document.querySelector(opts.stage) : opts.stage;
    if (src) stage.appendChild(src.content ? src.content.cloneNode(true) : src);
    if (GV.mountAll) GV.mountAll(stage);
    if (window.GVIcons) GVIcons.render(stage);
  }

  var rail = host.querySelector(".gv-bo-projrail");
  var active = opts.active;
  if (rail && !rail.__gvEditorWired) {
    rail.__gvEditorWired = 1;
    rail.addEventListener("click", function (e) {
      // Chevron on the Project row → disclose/collapse the utility group (the
      // GoVocal-current behaviour: no separate "Manage" toggle heading).
      if (e.target.closest('.gv-bo-projrail__item[data-view="project"] .gv-bo-projrail__chev')) {
        var g = rail.querySelector(".gv-bo-projrail__group");
      ...
      if (action) {
        ...
        if (act === "toggle-manage") {
          var g2 = rail.querySelector(".gv-bo-projrail__group");
          if (g2) g2.classList.toggle("is-collapsed");
          return;
        }
        // A page can intercept a nav intent (e.g. bo-project-phase switches its stage
        // to the "New phase"/"New survey" form in place instead of navigating).
        // opts.onNav(target, kind, e) → return true to say "handled, stop here".
        if (opts.onNav && opts.onNav(act, "action", e) === true) return;
        if (BO_EDITOR_NAV[act]) { location.href = BO_EDITOR_NAV[act]; return; }
      }
      var phaseRow = e.target.closest(".gv-bo-projrail__phase[data-phase]");
      if (phaseRow) {
        var pk = phaseRow.getAttribute("data-phase");
        if (opts.onNav && opts.onNav(pk, "phase", e) === true) return;
        if (pk !== active && BO_EDITOR_NAV[pk]) location.href = BO_EDITOR_NAV[pk];
        return;
      }
      var item = e.target.closest(".gv-bo-projrail__item[data-view]");
      if (item) {
        var v = item.getAttribute("data-view");
        if (opts.onNav && opts.onNav(v, "item", e) === true) return;
        if (v !== active && BO_EDITOR_NAV[v]) location.href = BO_EDITOR_NAV[v];
      }
    });
  }

  // Topbar: title → the project page, the Preview eye → the FO project page.
  if (!host.__gvTopbarWired) {
    host.__gvTopbarWired = 1;
    host.addEventListener("click", function (e) {
      if (e.target.closest(".gv-bo-topbar__title")) location.href = BO_EDITOR_NAV.project;
      else if (e.target.closest('.gv-bo-topbar__actions [aria-label="Preview project"]')) window.open("../project-page/", "_blank", "noopener");
    });
  }
  return host;
};
...
/* ── fo-phone-preview ─────────────────────────────────────────────────────────
 * Device-frame chrome that wraps a live preview of FO content. It owns NO content
 * of its own — it's a reusable viewport: a scaled screen + a hidden-scrollbar
 * scroller into which the caller drops canonical content (e.g. the Content Builder
 * render, GVWidgets.project.renderFO(model)). Because it carries no markup for the
 * components inside, the content stays single-source — it tracks canonical, never a
 * frozen copy. Mount it via GV.mount so the host is provenance-stamped.
 *
 *   variant  "phone" (default, 332×640) | "window" (1180×720)
 *   notch    boolean — show the phone notch (default: off, matching the light frame)
 *   header   raw HTML for a FIXED, non-scrolling top bar (e.g. a FO mobile navbar) | ""
 *   body     raw HTML slot — the scrolling content (canonical). Empty → empty state.
 *   emptyHtml  override the default empty-state markup
 *   label    aria-label for the frame
 */
GV.register("fo-phone-preview", function (props) {
  props = props || {};
  var esc = GV.esc;
  var variant = props.variant === "window" ? "window" : "phone";
  var notch = variant === "phone" && !!props.notch;
  var header = props.header != null ? props.header : "";
  var body = props.body != null ? props.body : "";
  var empty = !header && !String(body).trim();
  var emptyHtml = props.emptyHtml != null ? props.emptyHtml :
    '<div class="gv-fo-phone__empty">' +
      '<span class="gv-fo-phone__empty-ic" data-gv-icon="eye" aria-hidden="true"></span>' +
      '<div class="gv-fo-phone__empty-t">Nothing to preview yet</div>' +
      '<div class="gv-fo-phone__empty-s">Add content in the builder and it shows up here.</div>' +
    '</div>';
  var screen = empty
    ? emptyHtml
    : header + '<div class="gv-fo-phone__scroll">' + body + "</div>";
  return (
    '<div class="gv-fo-phone gv-fo-phone--' + variant + (empty ? " is-empty" : "") + '"' +
      (props.label ? ' aria-label="' + esc(props.label) + '" role="img"' : "") + ">" +
      (notch ? '<div class="gv-fo-phone__notch"></div>' : "") +
      '<div class="gv-fo-phone__screen">' + screen + "</div>" +
    "</div>"
  );
});

/* ── fo-managers ──────────────────────────────────────────────────────────────
 * The manager control row at the top of an FO project page (above the banner):
 * the admin-only "Edit project" affordance + the "Follow / Unfollow (n)" toggle.
 * This is the single source for that markup — GVWidgets.project.renderFO emits it
 * (gated on the admin model, so the phone/editor preview shows it while a resident
 * render omits the admin-only edit) and the fo-phone-preview gallery drops it into
 * its slot, so the preview and the real project page never carry a forked copy.
 * Right-aligned; wraps on a narrow phone. Buttons carry data-gv-fo-edit /
 * data-gv-fo-follow hooks so a host editor can wire real behaviour onto them.
 *
 *   edit       boolean — show "Edit project" (admin affordance). default: true
 *   following  boolean — initial follow state. default: true
 *   count      follower count shown in the toggle label. default: 9
 */
GV.register("fo-managers", function (props) {
  props = props || {};
  var esc = GV.esc;
  var edit = props.edit !== false;
  var following = props.following !== false;
  var count = props.count != null ? props.count : 9;
  var editBtn = edit
    ? '<a class="gv-btn secondary-outlined size-s" href="#" data-gv-fo-edit>' +
        '<span data-gv-icon="edit" aria-hidden="true"></span> Edit project</a>'
    : "";
  var label = (following ? "Unfollow (" : "Follow (") + count + ")";
  var followBtn =
    '<button class="gv-btn primary size-s" type="button" data-gv-fo-follow aria-pressed="' +
      (following ? "true" : "false") + '">' +
      '<span data-gv-icon="notification" aria-hidden="true"></span> ' +
      '<span class="gv-fo__managers-label">' + esc(label) + "</span></button>";
  return '<div class="gv-fo__managers">' + editBtn + followBtn + "</div>";
});

/* ── extra-survey ─────────────────────────────────────────────────────────────
 * A survey linked into a project page / the Content Builder, separate from the
 * phase's own method. Rendered as a full CARD (tag + status pill + title + desc +
 * CTA) or a bare BUTTON, in GoVocal primary (navy) or secondary (soft teal). The
 * temporal STATE drives the pill + CTA treatment. This is the single source for the
 * markup — GVWidgets.project's cbSurveyHTML delegates here, so the editor preview,
 * the FO project page, and this gallery all render the exact same component.
 *
 *   format  "card" (default) | "button"
 *   style   "primary" (default) | "secondary"   (only honoured in the live state)
 *   state   "current"(default) | "notstarted" | "past" | "taken" | "ineligible"
 *   name, desc   survey content (card only)
 *   label   custom CTA text — overrides the live-state CTA only
 *   empty   true → the "no survey linked yet" placeholder
 */
GV.register("extra-survey", function (props) {
  props = props || {};
  var esc = GV.esc;
  if (props.empty) {
    // Admin-only linked empty state: only the Project Editor / Content Builder sees
    // the "no survey linked yet" placeholder. A resident render collapses to nothing.
    if (!props.admin) return '';
    return '<div class="gv-extra-survey__empty">' +
      '<span class="gv-icon" data-gv-icon="survey" aria-hidden="true"></span>' +
      '<div class="gv-extra-survey__empty-t">No survey linked yet</div>' +
      '<div class="gv-extra-survey__empty-s">Pick a survey in the panel to show it on the page.</div>' +
      '<span class="gv-empty-adminhint"><span class="gv-icon" data-gv-icon="eye-off"></span>Only admins can see this</span></div>';
  }
  // state → treatment (pill + CTA copy + button style + disabled). m1/m2 meta rows
  // from the old layout are intentionally dropped — the card shows state via pill+CTA.
  var STATES = {
    current:    { pill: "open",     pillText: "Open",       cta: "Take the survey",                         btn: "primary",            dis: false, allowLabel: true  },
    notstarted: { pill: "upcoming", pillText: "Upcoming",   cta: "Opens 1 Jul",                             btn: "primary",            dis: true,  allowLabel: false },
    past:       { pill: "closed",   pillText: "Closed",     cta: "Closed 17 days ago",                      btn: "secondary-outlined", dis: true,  allowLabel: false },
    taken:      { pill: "done",     pillText: "Completed",  cta: '<span class="gv-icon" data-gv-icon="check"></span> Thanks — response received', btn: "secondary-outlined", dis: true, allowLabel: false },
    ineligible: { pill: "locked",   pillText: "Restricted", cta: '<span class="gv-icon" data-gv-icon="lock"></span> You’re not eligible to take this survey', btn: "secondary-outlined", dis: true, allowLabel: false }
  };
  var stt = STATES[props.state] || STATES.current;
  var ctaText = (stt.allowLabel && props.label && props.label.trim()) || stt.cta;
  var btnStyle = (props.state && props.state !== "current") ? stt.btn : (props.style === "secondary" ? "secondary-outlined" : "primary");
  var btn = '<a class="gv-btn full ' + btnStyle + '" href="#"' + (stt.dis ? ' aria-disabled="true"' : "") + ">" + ctaText + "</a>";
  if (props.format === "button") return '<div class="gv-extra-survey">' + btn + "</div>";
  return '<div class="gv-extra-survey gv-extra-survey--card">' +
    '<div class="es-head">' +
      ...
      '<span class="gv-statuspill ' + stt.pill + '">' + esc(stt.pillText) + "</span>" +
    "</div>" +
    '<h3 class="gv-extra-survey__title">' + esc(props.name || "Survey") + "</h3>" +
    '<p class="gv-extra-survey__desc">' + esc(props.desc || "") + "</p>" +
    btn + "</div>";
});
