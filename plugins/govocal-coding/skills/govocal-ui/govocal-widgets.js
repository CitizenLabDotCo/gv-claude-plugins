 ...
 *
 * DEPENDS ON govocal-instances.js — load it FIRST. The widget emitters compose canonical
 * components by delegating to GV.render (e.g. cbSurveyHTML -> GV.render('extra-survey')),
 * so `GV` must be defined when a widget renders. The two files are a pair (see MODEL.md).
 *
...
window.GVWidgets = (function () {
  ...
  // empty-state body for the HTML block widget (no 'code' glyph in the icon set → inline SVG)
  var HTML_EMPTY = '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M13.8 5 10.2 19M7.5 8.5 4 12l3.5 3.5M16.5 8.5 20 12l-3.5 3.5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg> HTML block<small>Paste your HTML code in the settings panel</small>';
  ...
  var HOMEPAGE = {
    ...
    'html': {
      ...
      make: function (d) {
        d = d || {};
        var code = (typeof d.html === 'string' && d.html) ? d.html : null;
        return '<section class="gv-section"><div class="gv-section__body"><div class="hb-html' + (code ? '' : ' is-empty') + '">' + (code || HTML_EMPTY) + '</div></div></section>';
      },
      settings: function (block) {
        var el = block.querySelector('.hb-html');
        var code = (el && !el.classList.contains('is-empty')) ? el.innerHTML : '';
        return '<div class="hb-warn"><span class="gv-icon" data-gv-icon="alert-circle"></span><span><b>WARNING: DO NOT PASTE CODE FROM UNTRUSTED SOURCES.</b> Custom HTML renders directly on your homepage — only paste code from providers you trust.</span></div>'
          + '<div class="hb-set-row"><label class="hb-set-label">HTML code</label><textarea class="gv-input hb-code" rows="10" data-hbhtml placeholder="&lt;div&gt;…&lt;/div&gt;">' + esc(code) + '</textarea></div>'
          + fText('Short description of the content', '', null, { multiloc: true, help: 'It is useful to provide this information for users who rely on a screen reader or other assistive technology.' });
      },
      wire: function (block, api) {
        var el = block.querySelector('.hb-html');
        var ta = api.panel.querySelector('[data-hbhtml]');
        if (el && ta) ta.addEventListener('input', function () {
          if (ta.value.trim()) { el.classList.remove('is-empty'); el.innerHTML = ta.value; }
          else { el.classList.add('is-empty'); el.innerHTML = HTML_EMPTY; if (window.GVIcons) GVIcons.render(el); }
        });
        bind(block, api.panel);
      }
    },
    'video-embed': {
      label: 'Video',
  ...
  var HOMEPAGE_ORDER = ['homepage-banner', 'spotlight', 'areas', 'open-to-participation', 'finished-or-archived', 'events', 'call-to-action', 'community-monitor-cta', 'text', 'selection', 'published', 'projects', 'followed-items', 'image-text-cards', 'two-column', 'three-column', 'image', 'button', 'accordion', 'iframe', 'html', 'video-embed', 'white-space'];
  ...
  var PROJECT = (function () {
    ...
    // ── Linked empty states (admin-only widget placeholders) ──
    // Shown in the Project Editor / Content Builder (canvas + phone preview) when a
    // widget is placed but has no content yet. Bespoke per widget, passive (no CTA),
    // each carrying the shared admin hint so admins know it isn't public. The `admin`
    // flag gates them: in a resident render (admin falsy) the empty widget collapses
    // to nothing instead. Events is the exception (its empty state is participant-facing).
    function adminHint(text) {
      return '<span class="gv-empty-adminhint"><span class="gv-icon" data-gv-icon="eye-off"></span>' + foEsc(text || 'Only admins can see this') + '</span>';
    }
    var HERO_EMPTY_SVG = '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><rect x="3" y="4" width="18" height="16" rx="2" stroke="currentColor" stroke-width="2"/><circle cx="8.5" cy="9.5" r="1.6" fill="currentColor"/><path d="M4.5 18l4.5-5 3.2 3.2L16 12l3.5 4.5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    function heroEmptyInner() {
      return '<div class="gv-hero-empty">' + HERO_EMPTY_SVG +
        '<span class="gv-hero-empty__label">Add a project image</span>' +
        adminHint('No image yet · only admins see this') + '</div>';
    }

    function cbTimelineHTML(admin) {
      if (!CB_PHASES.length) {
        if (!admin) return '';
        // Ghosted skeleton: a faded 3-step stepper + a phase-detail card, so it still
        // reads as a timeline (label + shapes + admin hint).
        var skStep = '<span class="gv-phases__skel-step"><span class="d"></span><span class="l"></span></span>';
        return '<section class="gv-phases cb-timeline gv-phases--empty"><div class="gv-phases__bar"><h2>Phases</h2></div>' +
          '<div class="gv-phases__empty">' +
          '<span class="gv-phases__empty-t">No phases yet</span>' +
          '<div class="gv-phases__skel-steps" aria-hidden="true">' + skStep + skStep + skStep + '</div>' +
          '<div class="gv-phases__skel-card" aria-hidden="true"><span class="gv-skel gv-skel--line w40"></span><span class="gv-skel gv-skel--line w25"></span><span class="gv-skel gv-skel--line w90"></span><span class="gv-skel gv-skel--line w70"></span></div>' +
          adminHint() + '</div></section>';
      }
    ...
    function cbEventsHTML(n, events) {
      ...
      // Empty state is the resident-facing one (GV.render('extra-survey') sibling): the
      // Events widget shows "no events scheduled" to participants too, so — unlike the
      // admin-only placeholders — it renders regardless of who's viewing and carries no
      // admin hint. Keep the section head so the widget still reads as "Events".
      if (!list.length) {
        return '<section class="cb-events"><div class="gv-events__head"><h2 class="gv-title h2" style="margin:0">Events</h2></div>' +
          (window.GV && GV.render ? GV.render('event-card', { variant: 'empty' }) :
            '<div class="gv-events__empty"><span data-gv-icon="calendar" aria-hidden="true"></span><p class="gv-text bodyM">No upcoming or ongoing events are currently scheduled.</p></div>') +
          '</section>';
      }
    ...
    function cbPboxDefault() {
      ...
      // `panel` is vestigial — the configuration panel is locked to the "By track" layout
      // (see PBOX_PANELS). The rendered module itself is fixed (direction A).
      if (PBOX_METHODS_OVERRIDE) return { panel: 'C', showParticipants: true, aggTitle: PBOX_AGG_OVERRIDE || 'Participate', methods: PBOX_METHODS_OVERRIDE.map(function (m) { return Object.assign({}, m); }) };
      // Timeline = the project's live phase method (ideation here). Currently-open and
      // upcoming items are ALWAYS extra surveys (they come from Extras).
      return { panel: 'C', showParticipants: true, aggTitle: 'Participate · 3 ways', methods: [
        ...
        { id:'m_qa',      type:'survey',   label:'Budget priorities survey',                    cta:'Take the survey',    dates:'Ongoing',         status:'active',   timeline:false, current:false, visible:true  },
        { id:'m_offline', type:'survey',   label:'Park use survey',                             cta:'Take the survey',    dates:'Ongoing',         status:'active',   timeline:false, current:false, visible:false },
        ...
        { id:'m_vote',    type:'survey',   label:'Public spaces survey',                        cta:'Opens 12 Jul',       dates:'12 – 26 Jul',     status:'upcoming', timeline:false, current:false, visible:true  },
    ...
    function cbPboxBodyHTML(st, admin) {
      var vis = st.methods.filter(function (m) { return m.visible; });
      var act = vis.filter(pbActive);
      // Linked empty state: no opportunities AND no participants → admin-only placeholder
      // (residents see the box collapse to nothing).
      if (!act.length && !st.showParticipants) {
        if (!admin) return '';
        // Ghosted skeleton: a faded button + participants, so it still reads as a
        // participation box (label + shapes + admin hint).
        return '<div class="gv-pbox gv-pbox--empty"><div class="gv-pbox__empty">' +
          '<span class="gv-pbox__empty-t">No ways to take part yet</span>' +
          '<div class="gv-pbox__skel" aria-hidden="true">' +
            '<span class="gv-skel gv-skel--btn"></span>' +
            '<div class="gv-pbox__skel-people"><span class="gv-skel gv-skel--av"></span><span class="gv-skel gv-skel--av"></span><span class="gv-skel gv-skel--av"></span><span class="gv-skel gv-skel--line"></span></div>' +
          '</div>' +
          adminHint() + '</div></div>';
      }
      var inner = act.length ? PBOX_DIRECTIONS.A.render(st, vis) : '';
    ...
    // The configuration panel is LOCKED to the "By track" layout (2026-06-23) — the
    // timeline (current-phase) method on top, then the currently-open extras, then
    // upcoming ones. Every method (incl. the timeline one) is toggleable; past methods
    // are intentionally hidden here. No status pill (the group already says open/upcoming)
    // and no Timeline flag/tint. The earlier A/B/D/E panel explorations were removed when
    // this was locked in. Each group title is a normal settings rowlabel with a small
    // .gv-bo-help description beneath it.
    var PBOX_PANELS = {
      C: { name: 'By track', render: function (st) {
        function row(m) {
          var i = st.methods.indexOf(m);
          return '<label class="cb-mrow">' +
            '<input type="checkbox" data-mi="' + i + '"' + (m.visible ? ' checked' : '') + '>' +
            '<span class="cb-mrow__body"><span class="cb-mrow__name">' + m.label + '</span>' +
            '<span class="cb-mrow__meta">' + pbTypeStr(m) + ' · ' + m.dates + '</span></span></label>';
        }
        function grp(label, desc, items) {
          if (!items.length) return '';
          return '<div class="cb-mgroup">' +
            '<span class="gv-bo-cb-settings__rowlabel cb-mgroup__h">' + label + '</span>' +
            (desc ? '<p class="gv-bo-help cb-mgroup__desc">' + desc + '</p>' : '') +
        ...
        return grp('Timeline', 'Follows the project timeline and updates on its own.', timeline) +
               grp('Currently open', 'Open surveys appear here.', openExtra) +
               grp('Upcoming', 'Future surveys appear here.', upcoming);
      } }
    };
    ...
    // Default = NO survey linked, so a freshly dropped Extra-surveys widget shows its
    // empty state until the admin picks a survey in the panel. Seeded demos call
    // _seedSurvey(block, id) to pre-link one.
    function cbSurveyState(block) { if (!SURVEY_STATE[block.id]) SURVEY_STATE[block.id] = { survey:null, format:'card', style:'primary', label:'', cardVariant:'easy', state:'current' }; return SURVEY_STATE[block.id]; }
    // Markup + temporal states now live in the canonical GV.render('extra-survey')
    // instance — this is a thin adapter: resolve the survey's name/desc (sample
    // content the editor owns) and hand off rendering so there's ONE definition.
    function cbSurveyHTML(survey, format, style, label, variant, state, dataOverride, admin) {
      if (!survey) return GV.render('extra-survey', { empty: true, admin: admin });
      ...
      return GV.render('extra-survey', { format: format, style: style, state: state, label: label, name: s.name, desc: s.desc });
    }
    ...
    var MK = {
      ...
      'participation-box': function () { return cbPboxBodyHTML(cbPboxDefault(), true); },
      'extra-surveys': function () { return cbSurveyHTML(null, 'card', 'primary', '', 'easy', 'current', null, true); },
      ...
      'timeline': function () { return cbTimelineHTML(true); },
      'events': function () { return cbEventsHTML(3); },
      'hero': function () { return '<div class="cb-hero cb-hero--empty">' + heroEmptyInner() + '</div>'; },
    ...
    function bodyFor(widget, block) {
      ...
      if (widget === 'participation-box') {
        // The configuration panel is locked to the "By track" layout (the module render
        // itself is final). A one-line description sits below the panel title; the
        // participants toggle pins to the top; the binder fills #cb-pbox-panel.
        return '<p class="gv-bo-help">The box on the project page where residents choose how to take part.</p>' +
      ...
      if (widget === 'timeline') { return '<p class="gv-bo-help">Phases are set in the <a href="#">project editor</a>. The timeline updates on its own as dates pass.</p>'; }
      // Events has no content settings of its own: the list is owned by the project
      // editor (Events). The panel is pure guidance pointing there — add / edit / remove
      // happen at the source and flow back into this module automatically.
      if (widget === 'events') return '<p class="gv-bo-help">Events are managed in the project editor, under <a href="#" data-cb-goto="events">Events</a>. Add, edit, or remove them there — they’ll appear here automatically.</p>';
      if (widget === 'extra-surveys') {
        return '<p class="gv-bo-help">Only surveys under Extras can be linked. Add them in the <a href="#">project editor</a>.</p>' +
          row('Survey', '<select class="gv-bo-select" data-set="survey"><option value="">Select a survey\u2026</option><option value="mobility">Mobility check-in</option></select>') +
    ...
    function bindParticipationBox(block, api) {
      ...
      function renderModule() { cbSetBody(block, cbPboxBodyHTML(st, true)); }   // module is fixed (direction A); canvas = admin
      ...
      function syncPanel() {
        if (!cont) return;
        cont.innerHTML = PBOX_PANELS.C.render(st) + footerHTML();
        ...
        cont.querySelectorAll('input[data-mi]:not([disabled])').forEach(function (inp) {
          inp.addEventListener('change', function () {
            ...
            renderModule(); syncPanel();   // footer (agg-title) follows the change
          });
        });
    ...
    function bindExtraSurveys(block, api) {
      ...
      st.cardVariant = 'easy';
      function render() { cbSetBody(block, cbSurveyHTML(st.survey, st.format, st.style, st.label, st.cardVariant, st.state, null, true)); }
    ...
    function wireBody(widget, block, api) {
      ...
      if (widget === 'timeline') { return; }   // settings is info-only — no controls to wire
      if (widget === 'events') return;   // no content settings — the events list is owned by the project editor
      if (widget === 'hero') {
        var heroWrap = bq('.cb-hero'), hf = pv('cb-f-herofile'), hu = pv('cb-f-herourl');
        function setHero(src) {
          if (!heroWrap) return;
          if (src) { heroWrap.classList.remove('cb-hero--empty'); heroWrap.innerHTML = '<img class="cb-hero__img" src="' + src + '" alt="Project banner">'; }
          else { heroWrap.classList.add('cb-hero--empty'); heroWrap.innerHTML = heroEmptyInner(); if (window.GVIcons) GVIcons.render(heroWrap); }
        }
        if (hf && heroWrap) hf.addEventListener('change', function () { if (hf.files && hf.files[0]) setHero(URL.createObjectURL(hf.files[0])); });
        if (hu && heroWrap) hu.addEventListener('input', function () { setHero(hu.value.trim()); });
        return;
      }
    ...
    function applyModel(model, frame) {
      ...
      if (!frame) return;
      Array.prototype.forEach.call(frame.querySelectorAll('[data-widget="timeline"]'), function (b) { cbSetBody(b, cbTimelineHTML(true)); wireTimeline(b); });
      Array.prototype.forEach.call(frame.querySelectorAll('[data-widget="events"]'), function (b) { cbSetBody(b, cbEventsHTML(3)); });
      Array.prototype.forEach.call(frame.querySelectorAll('[data-widget="participation-box"]'), function (b) {
        ...
        if (model.showParticipants != null) st.showParticipants = model.showParticipants;
        cbSetBody(b, cbPboxBodyHTML(st, true));
      });
    ...
    // Clean FO HTML for one block — no builder chrome. `admin` enables the linked
    // empty states (hero/pbox/extra-surveys/timeline) for editor surfaces; in a
    // resident render they collapse instead.
    function foBlock(type, data, admin) {
      data = data || {};
      switch (type) {
        ...
        case 'two-column': case 'three-column': return '<div class="gv-fo__cols gv-fo__cols--' + (type === 'three-column' ? 3 : 2) + '">' + (data.cols || []).map(function (col) { return '<div class="gv-fo__col">' + col.map(function (cb) { return foBlock(cb.type, cb.data, admin); }).join('') + '</div>'; }).join('') + '</div>';
        case 'iframe': return '<div class="gv-fo__embed">' + foEsc(data.cap || 'Embedded content') + '</div>';
        case 'participation-box': return cbPboxBodyHTML(data.pbox || cbPboxDefault(), admin);
        case 'extra-surveys': var sv = data.survey || { survey: 'mobility', format: 'card', style: 'primary', label: '', cardVariant: 'easy', state: 'current' }; return cbSurveyHTML(sv.survey, sv.format, sv.style, sv.label, sv.cardVariant, sv.state, sv.data, admin);
        case 'timeline': return cbTimelineHTML(admin);
        case 'events':
          ...
          return cbEventsHTML(data.n || (data.events ? data.events.length : 3), data.events);
        default: return '';
      }
    }
    ...
    function renderFO(model) {
      model = model || {};
      var admin = !!model.admin;          // editor surfaces (canvas / phone preview) → show linked empty states
      var blocks = model.blocks || [];
      var hero = '', hasHero = false, head = '', body = '';
      blocks.forEach(function (bl) {
        if (bl.type === 'hero') { hasHero = true; hero = (bl.data && bl.data.src) || hero; return; }
        if (bl.type === 'title') { head += foBlock('title', bl.data); return; }
        body += foBlock(bl.type, bl.data, admin);
      });
      // Filled → image banner. Empty + admin → the dashed "add a project image" placeholder.
      // Empty + resident → no banner at all (today's behaviour).
      var banner = hero ? '<div class="gv-fo__banner"><img src="' + foEsc(hero) + '" alt=""></div>'
        : (admin && hasHero ? '<div class="gv-fo__banner gv-fo__banner--empty">' + heroEmptyInner() + '</div>' : '');
      // Manager controls (Edit project + Follow) above the banner — the single
      // source is GV.render('fo-managers'). Shown on editor/preview surfaces
      // (admin), so the phone/window preview mirrors the real project page; a
      // plain resident render omits it (the host page owns its own follow CTA).
      var managers = (admin && window.GV && GV.render) ? GV.render('fo-managers', {}) : '';
      return '<div class="gv-fo">' + managers + banner + '<div class="gv-fo__pad">' + head + body + '</div></div>';
    }
    ...
    // Pre-link a survey on a seeded Extra-surveys block (the default is now empty).
    REG._seedSurvey = function (block, id) { var st = cbSurveyState(block); st.survey = id || null; cbSetBody(block, cbSurveyHTML(st.survey, st.format, st.style, st.label, st.cardVariant, st.state, null, true)); };
