...
- **If the monitor is not enabled, never leave this section empty**: state the absence as a finding (and a recommendation), then fall back to the period's open-text corpus — survey open answers first (usually the largest voice on survey-heavy platforms), then ideas and comments — AI-coded with the project-report protocol. State the corpus and its limits explicitly ("quotes from public ideas only; survey text not reachable" when that's the case, as in the Falkirk test).
...
- **GAP-N** 🌐 Cross-tenant benchmark service (country × population-band cohorts; Library scores API + participation-50% re-weighting) — i4 item 4, still unscoped; the one gap that cannot live in the tenant-scoped SQL layer by design. Sources today: Metabase (cross-tenant usage) + Project Library (browser-scraped). **Interim real path (proven in the Falkirk test, signed-in Metabase browser session required):** Metabase native queries against `production_cl2_back_raw` — `cl2_raw_tenants` (core_country_code, core_population, core_lifecycle_stage) defines the cohort (country × population ±2×), joined to `cl2_raw_users` (registrations/1k residents), `cl2_raw_impact_tracking_sessions`/`_pageviews` (reach, device, referrers, active staff via highest_role) and `cl2_raw_activities` (status changes, official feedback written, projects/phases created). This is the query set the benchmark service should productize. Reports using it stay INTERNAL TEST.
